...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: "assistant",
      content: "Cage, what's the move today in Atlanta? Let's stack some wins.",
    },
  ]);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      setPosition((prev) => ({
        x: Math.max(
          20,
          Math.min(window.innerWidth - 360, prev.x + e.movementX),
        ),
        y: Math.max(
          20,
          Math.min(window.innerHeight - 640, prev.y + e.movementY),
        ),
      }));
    };
  ...
  const toggleVoiceInput = () => {
    if (
      ...
      alert(
        "Your browser doesn't support voice input. Use Chrome for best results.",
      );
      return;
    }
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-zinc-700 bg-zinc-950">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-full flex items-center justify-center text-xs font-bold">
            CG
          </div>
          <div>
        ...
        <button
          ...
          className="p-2 hover:bg-zinc-800 rounded-xl no-drag"
        >
          ...
          >
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-zinc-950">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[82%] px-4 py-3 rounded-2xl text-[15px] ${
                      msg.role === "user"
