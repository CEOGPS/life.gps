...
description: >
  Explains the Desktop Commander MCP's capabilities for persistent shells/REPLs, long-running processes, broad filesystem access, safe surgical edits via edit_block, native Excel/DOCX/PDF handling, scalable search, SSH sessions, and process management. Includes concrete example workflows and strict usage conventions so agents can reliably compose its tools for real computer control. Use when the user runs /desktop-commander-overview or needs guidance on leveraging Desktop Commander MCP.
---
