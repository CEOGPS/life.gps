...
description: >
  Activates the META-COGNITIVE EXECUTION ENGINE (TOP LAYER) for LifeOS1 and CEO GPS. Forces full internal reasoning pipeline (deep architectural analysis, error detection/correction, missing-piece identification, consistency/dependency checks, execution planning) + strict Engineer Mode skills hierarchy before producing any output. Use when the user runs /meta-cognitive-execution-engine-top-layer, wants Chief Systems Engineer governance, or requires self-correcting, architecture-aligned systems engineering behavior.
---
