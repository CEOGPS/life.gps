...
test("French locale - type name translations in too_small errors", () => {
  ...
  if (!stringResult.success) {
    expect(stringResult.error.issues[0].message).toBe(
      "Trop petit : chaîne doit avoir >=5 caractères",
    );
  }
  ...
  if (!numberResult.success) {
    expect(numberResult.error.issues[0].message).toBe(
      "Trop petit : nombre doit être >=10",
    );
  }
  ...
  if (!arrayResult.success) {
    expect(arrayResult.error.issues[0].message).toBe(
      "Trop petit : tableau doit avoir >=3 éléments",
    );
  }
  ...
  if (!setResult.success) {
    expect(setResult.error.issues[0].message).toBe(
      "Trop petit : ensemble doit avoir >=2 éléments",
    );
  }
});
...
test("French locale - type name translations in too_big errors", () => {
  ...
  if (!stringResult.success) {
    expect(stringResult.error.issues[0].message).toBe(
      "Trop grand : chaîne doit avoir <=3 caractères",
    );
  }
  ...
  if (!numberResult.success) {
    expect(numberResult.error.issues[0].message).toBe(
      "Trop grand : nombre doit être <=10",
    );
  }
  ...
  if (!arrayResult.success) {
    expect(arrayResult.error.issues[0].message).toBe(
      "Trop grand : tableau doit avoir <=2 éléments",
    );
  }
});
...
test("French locale - type name translations in invalid_type errors", () => {
  ...
  if (!arrayResult.success) {
    expect(arrayResult.error.issues[0].message).toBe(
      "Entrée invalide : tableau attendu, objet reçu",
    );
  }
});
