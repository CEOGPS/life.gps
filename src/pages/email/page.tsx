    ...
    <div className="flex gap-3 min-h-0 flex-1">
      ...
      <div className="w-44 shrink-0 flex flex-col gap-3">
        <div className="glass rounded-xl border border-white/8 p-2">
          <div
            className="text-[9px] font-display tracking-widest px-2 mb-1.5"
            style={{ color: TEAL }}
          >
            ACCOUNTS
          </div>
          {ACCOUNTS.map((a) => (
            ...
            >
              <div
                className={`w-2 h-2 rounded-full ${a.color.split(" ")[0]}`}
              />
              <span className={`text-[11px] ${a.color.split(" ")[1]}`}>
                {a.label}
              </span>
              <div className="ml-auto w-1.5 h-1.5 rounded-full bg-white/15" />
            </div>
          ))}
          <button className="w-full text-center text-[9px] text-white/30 hover:text-primary/60 mt-1 py-1 font-display tracking-wider">
            + ADD ACCOUNT
          </button>
        </div>
        <div className="glass rounded-xl border border-white/8 p-2 flex-1">
          ...
          {FOLDERS.map((f) => (
            <div
              key={f.label}
              className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/5 cursor-pointer text-white/55 hover:text-white/80 transition-colors"
            >
              {f.icon}
              <span className="text-xs flex-1">{f.label}</span>
              {f.count > 0 && (
                <span className="text-[9px] glass-crimson px-1.5 rounded-full text-primary">
                  {f.count}
                </span>
              )}
            </div>
        ...
        <div className="flex-1 glass rounded-xl border border-white/8 flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="text-[10px] text-white/30 mt-1">
              Connect accounts to populate inbox
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 glass rounded-xl border border-white/8 flex items-center justify-center">
        <div className="text-center max-w-xs">
          ...
          <div className="text-[10px] text-white/30 mt-2 leading-relaxed">
            ...
            <br />
            Campaign analytics via SendGrid &amp; Brevo.
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex gap-4 flex-1 min-h-0">
      ...
      <div className="flex-1 flex flex-col gap-3 min-w-0">
        <div className="glass rounded-xl border border-white/8 p-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[9px] text-white/40 font-display tracking-wider block mb-1">
                CAMPAIGN NAME
              </label>
              <input
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg glass text-white/55 text-xs font-display hover:text-white/80 transition-all">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <div
              key={s.label}
              className="glass rounded-xl border border-white/8 p-3 text-center"
            >
              <div className="text-lg font-display text-white/80">{s.val}</div>
              <div
                className="text-[9px] font-display tracking-wider mt-1"
                style={{ color: TEAL }}
              >
                {s.label}
              </div>
      ...
      <div className="w-44 shrink-0 glass rounded-xl border border-white/8 p-3 flex flex-col gap-2">
        ...
        <button className="text-[9px] text-primary/60 font-display hover:text-primary transition-colors">
          + CREATE LIST
        </button>
      </div>
    </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          >
            ...
            <div
              className="text-[9px] font-display tracking-widest mt-1"
              style={{ color: TEAL }}
            >
              {s.label}
            </div>
      ...
      <div className="flex-1 glass rounded-xl border border-white/8 flex items-center justify-center min-h-[180px]">
        <div className="text-center">
          ...
          <div className="text-sm text-white/55">
            Connect email service to see analytics
          </div>
          <div className="text-[10px] text-white/30 mt-1">
            SendGrid, Brevo, Mailchimp supported
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="glass rounded-xl border border-white/8 p-4 flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            ...
            />
            <button className="px-3 py-1.5 rounded-lg glass-crimson text-primary text-[10px] font-display hover:glow-crimson-sm transition-all">
      ...
      <div className="glass rounded-xl border border-white/8 p-4 flex flex-col gap-3 flex-1">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="text-[10px] text-white/30 mt-1">
              Connect accounts to start email warm-up
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="glass rounded-xl border border-white/8 p-4 flex flex-col gap-2">
          ...
          <input
            ...
            className="h-8 px-3 text-xs bg-white/4 border border-white/8 rounded-lg text-white/80 placeholder:text-white/30 focus:outline-none focus:border-primary/40"
          />
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-3 flex-1">
      ...
      <div className="flex-1 glass rounded-xl border border-white/8 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-[10px] text-white/30 mt-1">
            Create a list or import contacts to get started
          </div>
        </div>
      </div>
    </div>
