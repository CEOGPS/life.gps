...
const DECK_THEMES = [
  {
    id: "corporate",
    label: "💼 Corporate Hell",
    desc: "Meetings, HR, and the slow death of the soul",
  },
  {
    id: "atlanta",
    label: "🍑 Atlanta Life",
    desc: "Traffic, heat, Chick-fil-A, and gentrification",
  },
  {
    id: "millennial",
    label: "😭 Millennial Pain",
    desc: "Debt, avocado toast, and existential dread",
  },
  {
    id: "social",
    label: "📱 Social Media",
    desc: "Influencers, LinkedIn, and clout chasing",
  },
  {
    id: "family",
    label: "🦃 Family Drama",
    desc: "Thanksgiving, group chats, and passive aggression",
  },
  {
    id: "startup",
    label: "🚀 Startup Chaos",
    desc: "Disruption, pivot culture, and burning cash",
  },
];
...
function buildBlackCardSystem(theme, difficulty) {
  const diffMap = {
    ...
    abyss:
      "deeply cynical, existentially horrifying, or so accurate it hurts — still no explicit content",
  };
          ...
          {[0, 1, 2].map((i) => (
            <div
              ...
              style={{
                ...
                borderRadius: "50%",
                background: C.white,
                animation: `cardPulse 1.2s ${i * 0.2}s ease-in-out infinite`,
              }}
            />
          ))}
        </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `2px solid ${selected ? C.gold : "transparent"}`,
        borderRadius: 12,
        padding: "12px 14px",
      ...
      {sorted.map((p, i) => (
        <div
          ...
          style={{
            ...
            padding: "5px 10px",
            borderRadius: 8,
            background: i === 0 ? `${C.gold}15` : "rgba(255,255,255,0.03)",
          }}
        >
          ...
          {czarIndex === players.indexOf(p) && (
            <span style={{ fontSize: 9, color: C.purple, fontWeight: 700 }}>
              CZAR
            </span>
          )}
...
export default function DarkCardGame({ onBack }) {
  ...
  function addPlayer() {
    ...
    setPlayers((p) => [
      ...p,
      { id: Date.now(), name: newName.trim(), score: 0 },
    ]);
    setNewName("");
  }
  function removePlayer(id) {
    if (players.length > 2) setPlayers((p) => p.filter((x) => x.id !== id));
  }
  ...
  const dealRound = useCallback(
    async (currentBlack) => {
      ...
      const hand = shuffled.slice(0, count).map((text, i) => ({
        text,
        playerId:
          nonCzarPlayers[i % nonCzarPlayers.length]?.id || players[0].id,
      }));
      setWhiteCards(hand);
      setGenLoading(false);
    },
  ...
  const aiGenerateRound = useCallback(async () => {
    ...
    // Generate black card
    const blackRaw = await invokeLLM({
      systemPrompt: buildBlackCardSystem(theme, difficulty),
      prompt: "Generate 1 dark humor black card now.",
    }).catch(() => "");
    const newBlack =
      blackRaw.trim().replace(/^["']|["']$/g, "") ||
      STARTER_BLACK[Math.floor(Math.random() * STARTER_BLACK.length)];
    setBlackCard(newBlack);

    // Generate white cards
    const whiteRaw = await invokeLLM({
    ...
    const whites = Array.isArray(parsed)
      ? parsed
      : STARTER_WHITE.sort(() => Math.random() - 0.5).slice(0, 10);
    setWhiteCards(
      whites.map((text, i) => ({
        text: String(text),
        playerId:
          nonCzarPlayers[i % Math.max(nonCzarPlayers.length, 1)]?.id ||
          players[0].id,
      })),
    );
    setGenLoading(false);
  ...
  async function submitPick() {
    ...
    const judgement = parseJSON(judgeRaw) || {
      score: 7,
      verdict: "Actually Funny",
      reaction: "I've seen worse. Which is saying something.",
    };
  ...
  useEffect(() => {
    if (screen === "game" && blackCard === "") dealRound(null);
  }, [screen]);
  ...
  if (screen === "setup")
    return (
      ...
      >
        <div
          style={{
            background: `linear-gradient(135deg, #0a0b12, #1a1020)`,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "#6aaedd" }}>
                A dark humor party game for deeply flawed people
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ maxWidth: 580, margin: "0 auto" }}>
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 28,
                        borderRadius: "50%",
                        background: `${[C.gold, C.purple, C.teal, C.orange, C.blue, C.red][i % 6]}20`,
                        border: `1px solid ${[C.gold, C.purple, C.teal, C.orange, C.blue, C.red][i % 6]}50`,
                        display: "flex",
                        alignItems: "center",
                ...
                {DECK_THEMES.map((t) => (
                  <button
                    ...
                    style={{
                      padding: "10px 12px",
                      borderRadius: 10,
                      border: `1px solid ${theme === t.id ? C.white : "rgba(255,255,255,0.08)"}`,
                      background:
                        theme === t.id
                          ? "rgba(255,255,255,0.1)"
                          : "transparent",
                      color: theme === t.id ? C.white : "#6aaedd",
                      cursor: "pointer",
                      textAlign: "left",
                    }}
                  >
                    ...
                    <div
                      style={{ fontSize: 9, color: "#4a5568", marginTop: 2 }}
                    >
                      {t.desc}
                    </div>
                  </button>
            ...
            >
                ...
                {DIFFICULTY.map((d) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: "8px 10px",
                      borderRadius: 8,
                      border: `1px solid ${difficulty === d.id ? C.red : "rgba(255,255,255,0.06)"}`,
                      background:
                        difficulty === d.id ? `${C.red}15` : "transparent",
                      color: difficulty === d.id ? C.red : "#8892a4",
                      fontSize: 12,
                      cursor: "pointer",
                      textAlign: "left",
              ...
              >
                ...
                <div
                  style={{ fontSize: 12, color: "#6aaedd", marginBottom: 8 }}
                >
                  First to
                </div>
                  ...
                  {[5, 7, 10, 15].map((n) => (
                    <button
                      ...
                      style={{
                        padding: "8px 14px",
                        borderRadius: 8,
                        border: `1px solid ${winScore === n ? C.gold : "rgba(255,255,255,0.08)"}`,
                        background:
                          winScore === n ? `${C.gold}15` : "transparent",
                        color: winScore === n ? C.gold : "#8892a4",
                        fontSize: 13,
                        fontWeight: 700,
                        cursor: "pointer",
                      }}
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  <button
                    onClick={() => setShowDeckBuilder((b) => !b)}
                    style={btnS(C.purple)}
                  >
                ...
                >
                  <div>
                    ...
                    <button
                      ...
                      style={{ ...btnS(C.white), marginTop: 6 }}
                    >
                      Add Black Cards ({customBlack.length})
                    </button>
                  </div>
                  <div>
                    <div
                    ...
                    <button
                      ...
                      style={{ ...btnS(C.white), marginTop: 6 }}
                    >
                      Add White Cards ({customWhite.length})
                    </button>
                  </div>
                </div>
              </div>
  ...
  if (screen === "results") {
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 14, color: "#6aaedd", marginBottom: 24 }}>
          with {champion.score} black cards of pure darkness
        </div>
        <div
          style={{
            background: "#11131f",
        ...
        {history.length > 0 && (
          ...
          >
            ...
            {history.slice(-5).map((h, i) => (
              <div
                key={i}
                style={{ marginBottom: 8, fontSize: 11, color: "#8892a4" }}
              >
                <span style={{ color: C.white }}>"{h.white}"</span> →{" "}
                <span style={{ color: C.gold }}>{h.winner}</span>
                {h.verdict && (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }} />
        <button
          onClick={aiGenerateRound}
          disabled={genLoading}
          style={{ ...btnS(C.purple), opacity: genLoading ? 0.5 : 1 }}
        >
          {genLoading ? "⚫ Generating..." : "🤖 AI Round"}
        </button>
        <button onClick={nextRound} style={btnS("#4a5568")}>
          Skip →
        </button>
        <button onClick={() => setScreen("setup")} style={btnS(C.red)}>
          End
        </button>
      </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          <div
            style={{
              ...
              borderRadius: 10,
              background:
                phase === "winner" ? `${C.gold}10` : "rgba(255,255,255,0.04)",
              border: `1px solid ${phase === "winner" ? C.gold : "rgba(255,255,255,0.06)"}`,
            }}
          >
            <div
              style={{
                fontSize: 12,
                color: phase === "winner" ? C.gold : "#6aaedd",
                fontWeight: 600,
              }}
            >
              {phase === "play" &&
                `${czar?.name} (Czar): Pick the funniest answer. Everyone else: choose your submission.`}
              {phase === "reveal" && "Judging in progress..."}
              {phase === "winner" &&
                winner &&
                `🏆 ${czar?.name} chose "${whiteCards[selectedCard]?.text}" — ${winner.name} scores a black card!`}
            </div>
          </div>
          ...
          {phase === "play" && (
            <button
              ...
              style={{
                padding: "12px 28px",
                borderRadius: 12,
                background: selectedCard !== null ? C.gold : "#1a1e2e",
                border:
                  selectedCard !== null
                    ? "none"
                    : "1px solid rgba(255,255,255,0.08)",
                color: selectedCard !== null ? C.black : "#4a5568",
                fontSize: 13,
                fontWeight: 800,
                cursor: selectedCard !== null ? "pointer" : "not-allowed",
              }}
            >
            ...
            <div style={{ marginTop: 14 }}>
              ...
              {history
                ...
                .map((h, i) => (
                  ...
                  >
                    <div style={{ color: "#8892a4", fontStyle: "italic" }}>
                      "{h.white.slice(0, 40)}
                      {h.white.length > 40 ? "..." : ""}"
                    </div>
