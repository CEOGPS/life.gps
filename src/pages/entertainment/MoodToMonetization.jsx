  ...
  return (
    <button
      ...
      style={{
        padding: "6px 14px",
        borderRadius: 20,
        border: `1px solid ${selected ? C.pink : "rgba(255,255,255,0.1)"}`,
        background: selected ? `${C.pink}20` : "transparent",
        color: selected ? C.pink : "#8892a4",
        fontSize: 12,
        cursor: "pointer",
    ...
    >
      ...
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${col}15` }}>
            ...
            >
              ...
              <span
                style={{
                  ...
                  padding: "2px 9px",
                  borderRadius: 10,
                  background: `${effortColors[opp.effortLevel]}15`,
                  color: effortColors[opp.effortLevel],
                }}
              >
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          {opp.happinessBoost > 0 && (
            <HappinessBoost pct={opp.happinessBoost} />
          )}
          {opp.timeToLaunch && (
            <span style={{ fontSize: 11, color: "#6aaedd" }}>
              ⏱ Launch in {opp.timeToLaunch}
            </span>
          )}
          {opp.locationSuggestion && (
            <span style={{ fontSize: 11, color: "#6aaedd" }}>
              📍 {opp.locationSuggestion}
            </span>
          )}
        ...
        {TABS.map((t) => (
          <button
            ...
            style={{
              ...
              fontSize: 10,
              fontWeight: 600,
              color: tab === t ? col : "#4a5568",
              borderBottom:
                tab === t ? `2px solid ${col}` : "2px solid transparent",
            }}
          >
            {TAB_LABELS[t]}
          </button>
      ...
      <div style={{ padding: "14px 16px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {opp.calendarNote && (
              <div
                style={{
                  fontSize: 11,
                  color: opp.calendarConflict ? C.red : C.teal,
                }}
              >
        ...
        {tab === "promo" && opp.promoPost && (
          <div>
            ...
            <button
              onClick={() => copy(opp.promoPost, "promo")}
              style={{
                padding: "6px 16px",
                borderRadius: 8,
                background: copied === "promo" ? `${C.teal}20` : `${C.pink}15`,
                border: `1px solid ${copied === "promo" ? C.teal : C.pink}40`,
                color: copied === "promo" ? C.teal : C.pink,
                fontSize: 11,
                cursor: "pointer",
                fontWeight: 600,
              }}
          ...
          <div>
            {opp.emailSubject && (
              <div style={{ marginBottom: 10 }}>
                ...
                >
                  ...
                  <button
                    onClick={() => copy(opp.emailSubject, "subj")}
                    style={{
                      background: "none",
                      border: "none",
                      color: copied === "subj" ? C.teal : "#6aaedd",
                      fontSize: 11,
                      cursor: "pointer",
                    }}
                  >
            ...
            {opp.emailBody && (
              <div>
                ...
                <button
                  onClick={() => copy(opp.emailBody, "body")}
                  style={{
                    padding: "6px 16px",
                    borderRadius: 8,
                    background:
                      copied === "body" ? `${C.teal}20` : `${C.blue}15`,
                    border: `1px solid ${copied === "body" ? C.teal : C.blue}40`,
                    color: copied === "body" ? C.teal : C.blue,
                    fontSize: 11,
                    cursor: "pointer",
                    fontWeight: 600,
                  }}
...
export default function MoodToMonetization({ onBack }) {
  ...
  const runBridge = useCallback(async () => {
    ...
    try {
      const prompt = [
        ...
        creativeIn.description &&
          `What I've been doing: ${creativeIn.description}`,
        creativeIn.timeSpent &&
          `Time invested this week: ${creativeIn.timeSpent}`,
        creativeIn.network.length &&
          `My warm network: ${creativeIn.network.join(", ")}`,
      ...
      if (!parsed) {
        setError("Couldn't parse opportunities. Try again.");
        setLoading(false);
        return;
      }
      setResult(parsed);
  ...
  const runPulse = useCallback(async () => {
    ...
    const summary = log
      ...
      .map(
        (e) =>
          `${e.date}: ${e.type}${e.mood ? " (" + e.mood + ")" : ""}${e.notes ? " — " + e.notes : ""}`,
      )
      .join("\n");
    const raw = await invokeLLM({
      systemPrompt: PULSE_SYSTEM,
      prompt: `Creative log (last 14 entries):\n${summary}`,
    });
  ...
  const toggleNet = (n) =>
    setCreativeIn((c) => ({
      ...c,
      network: c.network.includes(n)
        ? c.network.filter((x) => x !== n)
        : [...c.network, n],
    }));
    ...
    >
      ...
      <div
        style={{
          ...
          borderBottom: `1px solid ${C.pink}25`,
          padding: "14px 20px",
          flexShrink: 0,
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: C.pink }}>
              Mood-to-Monetization Bridge
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd" }}>
              Convert creative energy into warm revenue opportunities
            </div>
          </div>
          {log.length > 0 && (
            <div style={{ display: "flex", gap: 14 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: C.pink }}>
                  {log.length}
                </div>
        ...
        {TABS.map((t) => (
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              whiteSpace: "nowrap",
              background: activeTab === t.id ? `${C.pink}20` : "transparent",
              color: activeTab === t.id ? C.pink : "#6aaedd",
              borderBottom:
                activeTab === t.id
                  ? `2px solid ${C.pink}`
            ...
            {!result ? (
              <>
                ...
                <div style={{ ...sCard, marginBottom: 14 }}>
                  ...
                  >
                    STEP 2 — WHAT YOU'VE BEEN CREATING
                  </div>
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    <div>
                      ...
                      >
                        Describe what you've been making
                      </label>
                      <input
                        value={creativeIn.description}
                        onChange={(e) =>
                          setCreativeIn((c) => ({
                            ...c,
                        ...
                        placeholder="e.g. Weekend guitar sessions, writing song hooks..."
                        style={inp}
                      />
                    </div>
                    <div>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            padding: "5px 12px",
                            borderRadius: 20,
                            border: `1px solid ${active ? C.purple : "rgba(255,255,255,0.1)"}`,
                            background: active
                              ? `${C.purple}20`
                              : "transparent",
                            color: active ? C.purple : "#8892a4",
                            fontSize: 11,
                            cursor: "pointer",
                            fontWeight: active ? 700 : 400,
                          }}
                    ...
                    {[
                      {
                        key: "schedule",
                        label: "📅 Schedule",
                        ph: "e.g. Weekends free, weekday evenings",
                      },
                      {
                        key: "financial",
                        label: "💰 Finances",
                        ph: "e.g. Looking for $2k side income",
                      },
                      {
                        key: "goals",
                        label: "🎯 Goals",
                        ph: "e.g. Launch a workshop by next month",
                      },
                    ].map((f) => (
                      <div key={f.key}>
                        <label
                ...
                <button
                  onClick={runBridge}
                  disabled={loading || !mood || creativeIn.types.length === 0}
                  style={{
                    ...btnP,
                    width: "100%",
                    padding: 13,
                    fontSize: 13,
                    opacity: mood && creativeIn.types.length > 0 ? 1 : 0.4,
                  }}
                >
                  {loading
                    ? "🌉 Building your bridge..."
                    : "🌉 Find My Monetization Opportunities"}
                </button>
              </>
            ) : (
              /* Results */
              <div>
                ...
                {result.opportunities?.length > 0 && (
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: 14, fontWeight: 800, color: C.gold }}
                    >
                      $
                      {result.opportunities
                        .reduce((s, o) => s + (o.revenueProjected || 0), 0)
                        .toLocaleString()}
                    </span>
                    <span style={{ fontSize: 11, color: "#4a5568" }}>
                      across {result.opportunities.length} opportunities
                    </span>
                    <button
          ...
          <div>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 11, color: "#6aaedd" }}>
                  Track your creative sessions to unlock pattern insights
                </div>
              </div>
              <button
            ...
            {showForm && (
              ...
              >
                ...
                >
                  <div>
                    ...
                    <select
                      value={logForm.type}
                      onChange={(e) =>
                        setLogForm((f) => ({ ...f, type: e.target.value }))
                      }
                  ...
                  <div>
                    ...
                    <select
                      value={logForm.mood}
                      onChange={(e) =>
                        setLogForm((f) => ({ ...f, mood: e.target.value }))
                      }
                      style={{ ...inp, appearance: "none" }}
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="What did you make/do?"
                      style={inp}
                    />
                  </div>
                </div>
                <button
                  onClick={addLog}
                  disabled={!logForm.type}
                  style={{ ...btnP, opacity: logForm.type ? 1 : 0.4 }}
                >
                  Save Activity
                </button>
              </div>
            ...
            {log.length === 0 ? (
              ...
              >
                ...
                <div>
                  Log your first creative activity to start tracking patterns.
                </div>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {entry.mood && (
                            <span style={{ fontSize: 10, color: C.pink }}>
                              {entry.mood}
                            </span>
                          )}
        ...
        {activeTab === "pulse" && (
          <div style={{ maxWidth: 580 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 20 }}>
              AI analysis of your logged creative patterns and untapped income
              signals
            </div>
            ...
            {log.length < 2 ? (
              ...
              >
                ...
                <div style={{ lineHeight: 1.6 }}>
                  Log at least 2 creative activities to unlock pulse analysis.
                </div>
              </div>
            ) : !pulse ? (
              <div style={{ textAlign: "center", padding: 40 }}>
                ...
                <div
                  style={{ fontSize: 13, color: "#c8c8d0", marginBottom: 20 }}
                >
                  Analyze your {log.length} logged activities to surface
                  patterns and hidden monetization signals.
                </div>
                <button
                  onClick={runPulse}
                  disabled={pulseLoad}
              ...
              >
                ...
                >
                  {[
                    {
                      label: "🎯 DOMINANT TYPE",
                      value: pulse.dominantCreativeType,
                      col: C.orange,
                    },
                    {
                      label: "⭐ PEAK MOOD FOR MONEY",
                      value: pulse.peakMoodForMoney,
                      col: C.gold,
                    },
                  ].map(({ label, value, col }) => (
                    <div
                      key={label}
                      style={{ ...sCard, border: `1px solid ${col}25` }}
                    >
                      <div
                        style={{
                          fontSize: 10,
                ...
                {[
                  {
                    ...
                    value: pulse.energyPattern,
                    col: C.blue,
                  },
                  {
                    label: "💎 UNTAPPED STRENGTH",
                    value: pulse.untappedStrength,
                    col: C.teal,
                  },
                  {
                    label: "📊 WEEKLY INSIGHT",
                    value: pulse.weeklyInsight,
                    col: C.purple,
                  },
                ].map(({ label, value, col }) =>
                  value ? (
                    <div
                      key={label}
                      style={{ ...sCard, borderLeft: `3px solid ${col}` }}
                    >
                      <div
                        style={{
                          fontSize: 10,
                ...
                <button
                  ...
                  style={{ ...btnS(C.pink), alignSelf: "flex-start" }}
                >
                  Re-analyze
                </button>
              </div>
