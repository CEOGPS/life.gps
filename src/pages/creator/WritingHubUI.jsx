...
import {
  ...
  exportToMarkdown,
  exportToPlainText,
} from "@/lib/writingTemplatesService";
...
export default function WritingHubUI({ onSaveDocument, onClose }) {
  ...
  const handleExportDocument = (format) => {
    ...
    switch (format) {
      case "html":
        content = generateGoogleDocsHTML(
          currentDoc.title,
          currentDoc.content,
          "Unknown",
        );
            ...
            {MODES.map((m) => (
              <button
                ...
                style={{
                  ...
                  background:
                    mode === m.id
                      ? `${m.id === "blog" ? C.blue : m.id === "resume" ? "#ff6b6b" : m.id === "contract" ? C.purple : m.id === "legal" ? C.orange : m.id === "social" ? C.pink : C.teal}20`
                      : "rgba(255,255,255,0.03)",
                  border:
                    mode === m.id
                      ? `1px solid ${m.id === "blog" ? C.blue : m.id === "resume" ? "#ff6b6b" : m.id === "contract" ? C.purple : m.id === "legal" ? C.orange : m.id === "social" ? C.pink : C.teal}50`
            ...
            {TONES.map((t) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  background:
                    tone === t.id ? `${C.orange}20` : "rgba(255,255,255,0.03)",
                  border:
                    tone === t.id
      ...
      >
        ...
        <div style={{ background: "#1a1b2a", padding: 16, borderRadius: 8 }}>
          ...
          <div style={{ fontSize: 12, color: "#6aaedd", marginBottom: 12 }}>
            {selectedMode?.icon} <strong>{selectedMode?.name}</strong> •{" "}
            {selectedTone?.icon} <strong>{selectedTone?.name}</strong>
          </div>
          <div style={{ fontSize: 11, color: "#6aaedd", lineHeight: 1.6 }}>
            Create, edit, and export professional documents in multiple formats.
            Use AI to generate content or start from templates.
          </div>
        </div>
          ...
          {[
            {
              ...
              desc: "Bold, italic, headings, lists, tables",
            },
            {
              icon: "🖼️",
              title: "Media Support",
              desc: "Add images, videos, and embedded content",
            },
            {
              icon: "🎨",
              title: "6 Writing Modes",
              desc: "Blog, Resume, Contract, Legal, Social, Marketing",
            },
            {
              icon: "🎯",
              title: "6 Tones",
              desc: "Persuasive, Informative, Professional, Casual, Funny, Summary",
            },
            {
              icon: "💾",
              title: "Multi-Format Export",
              desc: "HTML, Markdown, Plain Text, PDF coming soon",
            },
            {
              icon: "📚",
              title: "Document History",
              desc: "Access all your previously written documents",
            },
          ].map((feature, idx) => (
            <div
              key={idx}
