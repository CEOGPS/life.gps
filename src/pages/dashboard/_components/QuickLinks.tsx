        ...
        {links.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {links.map((l) => (
              <div
                key={l.id}
                className="flex items-center gap-1.5 glass rounded p-2 border border-white/5 group hover:border-primary/20 transition-colors"
              >
                <a
                  href={l.url}
                  target="_blank"
      ...
      {adding ? (
        <div className="space-y-1.5 border-t border-white/5 pt-2">
          ...
          <div className="flex gap-1.5">
            <button
              onClick={add}
              className="flex-1 h-6 text-[10px] rounded glass-crimson text-primary font-display"
            >
              ADD
            </button>
            <button
