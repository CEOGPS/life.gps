...
export default function LifeHacks() {
  const [hacks, setHacks] = usePersistentState<string[]>(
    "dashboard_life_hacks",
    [],
  );
  ...
  const display = hacks.length
    ? hacks
    : [
        "Daily life hack loads here...",
        "Click New Hacks to generate...",
        "Powered by your Worker's AI fallback chain...",
      ];
    ...
    <div className="flex flex-col gap-3 h-full">
      <div className="flex-1 space-y-2 overflow-y-auto">
        {display.map((hack, i) => (
          <div
            key={i}
            className="flex items-start gap-2 p-2.5 glass rounded-lg border border-white/5"
          >
            <Lightbulb
              size={12}
              className="text-yellow-400/40 mt-0.5 shrink-0"
            />
            <span
              className={`text-[11px] ${hacks.length ? "text-white/70" : "text-white/25 italic"}`}
            >
              {hack}
            </span>
          </div>
