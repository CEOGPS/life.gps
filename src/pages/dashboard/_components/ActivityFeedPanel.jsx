...
// Minimal Icon replacement — renders a text symbol at the requested size.
...
export default function ActivityFeedPanel() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([kvGet(KV_EVENTS), kvGet(KV_DISMISSED)]).then(
      async ([saved, dismissed]) => {
        if (cancelled) return;
        dismissedRef.current = new Set(
          Array.isArray(dismissed) ? dismissed : [],
        );
        const base = (Array.isArray(saved) ? saved : []).filter(
          (e) => !dismissedRef.current.has(e.id),
        );
        // Real source: scheduled social posts from the Worker queue.
        let live = [];
        try {
          const queue = await fetch(`${WORKER}/api/social/queue`)
            .then((r) => r.json())
            .catch(() => null);
          ...
          /* offline — show persisted only */
        }
        const seen = new Set(base.map((e) => e.id));
        const merged = [
          ...base,
          ...live.filter(
            (e) => !seen.has(e.id) && !dismissedRef.current.has(e.id),
          ),
        ];
        setEvents(merged);
        setConnected(true);
  ...
  useEffect(() => {
    if (loaded) kvSet(KV_EVENTS, events);
  }, [events, loaded]);
  ...
  function markRead(id) {
    setEvents((e) =>
      e.map((ev) => (ev.id === id ? { ...ev, read: true } : ev)),
    );
  }
    ...
    >
          ...
          >
            <span
              style={{
                ...
                height: 7,
                borderRadius: "50%",
                background: connected ? C.teal : C.orange,
          ...
          return (
            <button
              ...
              style={{
                ...
                boxShadow: isActive
                  ? `0 0 10px ${cfg ? cfg.color : C.blue}66`
                  : "none",
      ...
      >
        {[
          {
            label: "Payments",
            val: events.filter((e) => e.source === "stripe").length,
            color: "#635bff",
            icon: "💳",
          },
          {
            label: "Commits",
            val: events.filter((e) => e.source === "github").length,
            color: "#ffffff",
            icon: "📂",
          },
          {
            label: "Emails",
            val: events.filter((e) => e.source === "gmail").length,
            color: "#ea4335",
            icon: "📬",
          },
          {
            label: "Leads",
            val: events.filter((e) => e.source === "crm").length,
            color: C.orange,
            icon: "👥",
          },
        ].map((s) => (
            ...
            <div>
              <div
                style={{
                  ...
                  fontWeight: 800,
                  color: s.color,
                  textShadow: `0 0 12px ${s.color}99`,
                }}
              >
                {s.val}
              </div>
