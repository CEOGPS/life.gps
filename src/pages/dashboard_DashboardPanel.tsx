...
type InputMap = Record<string, string | number>;
...
const GridCard = ({ title, icon, actions, children, style: styleProp, gridColumn }: GridCardProps) => {
...
function YouTubePlayer() {
  ...
  async function search(q: string, overrideKey?: string) {
...
export default function DashboardPanel({ setActive }) {
  ...
  const [productInputs, setProductInputs] = useState<InputMap>({});
  ...
  const [financeInputs, setFinanceInputs] = useState<InputMap>({});
  const [editCredit, setEditCredit] = useState(false);
  const [credInput, setCredInput] = useState<InputMap>({});
