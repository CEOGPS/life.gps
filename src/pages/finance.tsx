...
import MarketTicker from "./finance_MarketTicker.tsx";
  ...
  return (
    <div className="flex gap-4 flex-1 min-h-0">
      ...
      <div className="w-64 shrink-0 space-y-2 overflow-y-auto">
        <div
          className="text-[9px] font-display tracking-widest mb-1"
          style={{ color: TEAL }}
        >
          CONNECTED ACCOUNTS
        </div>
        {ACCOUNTS.map((acc) => (
          <div
            key={acc.name}
            className="glass rounded-xl p-3 border border-white/8 hover:border-primary/20 transition-colors cursor-pointer"
          >
            <div className="flex items-center gap-2">
              ...
              <div className="text-right">
                <div className="text-xs text-white/55 font-display">
                  ${acc.balance}
                </div>
                {acc.trend === "up" && (
                  <TrendingUp
                    size={9}
                    className="text-emerald-400/60 ml-auto"
                  />
                )}
                {acc.trend === "down" && (
                  <TrendingDown size={9} className="text-primary/60 ml-auto" />
                )}
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col gap-3 min-w-0">
        {/* Live markets: crypto + stocks + AI insight */}
        <MarketTicker />
        ...
        <div className="flex-1 glass rounded-xl border border-white/8 flex items-center justify-center min-h-[120px]">
          <div className="text-center">
            ...
            <div className="text-sm text-white/55">
              Connect accounts to view growth charts
            </div>
            <div className="text-xs text-white/30 mt-1">
              Quarterly &amp; monthly breakdowns, AI insights
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="glass rounded-xl border border-white/8 p-3 flex items-center gap-2">
            ...
            <button className="ml-auto text-[10px] text-primary/60 font-display hover:text-primary transition-colors">
              CREATE
            </button>
          </div>
          <div className="glass rounded-xl border border-white/8 p-3 flex items-center gap-2">
      ...
      <div className="flex-1 glass rounded-xl border border-white/8 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-[10px] text-white/30 mt-1">
            Connect Stripe / PayPal to sync product revenue
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-4 flex-1">
      <div className="grid grid-cols-3 gap-3">
        {bureaus.map((b) => (
          <div
            key={b.name}
            className="glass rounded-xl border border-white/8 p-4 text-center"
          >
            <div
              className="text-[9px] font-display tracking-widest mb-2"
              style={{ color: TEAL }}
            >
              {b.name.toUpperCase()}
            </div>
            <div className="text-3xl font-display text-white/80">
              {b.score.split("/")[0]}
            </div>
      ...
      <div className="glass rounded-xl border border-white/8 p-4 flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <div
            ...
            style={{ color: TEAL }}
          >
            AI CREDIT REPAIR TIPS
          </div>
        </div>
        <div className="text-xs text-white/30">
          Connect Credit Karma or Experian to get personalized AI tips.
        </div>
...
function TabAccounts() {
  return (
    <div className="flex flex-col gap-4 flex-1">
        ...
        {ACCOUNTS.map((acc) => (
          <div key={acc.name} className="glass rounded-xl p-3 border border-white/8 hover:border-primary/20 transition-colors cursor-pointer">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs text-white/80 font-medium truncate">{acc.name}</div>
                <div className="text-[9px] text-white/40">{acc.type}</div>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <div className="text-xs font-display text-white/55 font-medium">${acc.balance}</div>
              {acc.trend === "up" && <TrendingUp size={10} className="text-emerald-400" />}
              {acc.trend === "down" && <TrendingDown size={10} className="text-primary" />}
              {acc.trend === "neutral" && <span className="text-white/30 text-[9px]">—</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex-1 glass rounded-xl border border-white/8 flex items-center justify-center min-h-[120px]">
        <div className="text-center">
          <Link size={28} className="mx-auto text-white/8 mb-3" />
          <div className="text-sm text-white/55 mb-2">Connect Plaid, Yodlee, or manual entry</div>
          <div className="text-xs text-white/30">Real-time balances, transaction sync, net worth tracking</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-4 flex-1">
      <div className="glass rounded-xl border border-white/8 p-4">
        <div
          className="text-[9px] font-display tracking-widest mb-3"
          style={{ color: TEAL }}
        >
          PORTFOLIO OVERVIEW
        </div>
      ...
      <div className="flex-1 glass rounded-xl border border-white/8 p-4 flex flex-col gap-2">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="text-[10px] text-white/30 mt-1">
              Connect brokerage to see insights
            </div>
          </div>
        </div>
      </div>
