...
function pullGameContext() {
  ...
  const recent = sessions
    ...
    .map(
      (s) =>
        `${s.game} (${s.durationMin}min, mood: ${s.moodAfter || "unknown"})`,
    )
    .join(", ");
...
function pushAuditInsight(insight) {
  ...
  const entry = {
    id: Date.now(),
    source: "Narrative Conflict Engine",
    insight,
    ...
    <div style={{ marginBottom: 8 }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: stat.color }}>
            {clamped}
          </span>
        </div>
      </div>
...
function ChoiceCard({ option, onSelect, disabled }) {
  const riskColor =
    { low: C.teal, medium: C.orange, high: C.red }[option.riskLevel] ||
    C.orange;
  const impacts = Object.entries(option.statImpacts || {}).filter(
    ([, v]) => v !== 0,
  );
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        if (!disabled) e.currentTarget.style.borderColor = riskColor;
        e.currentTarget.style.background = `${riskColor}12`;
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.borderColor = `${riskColor}30`;
        e.currentTarget.style.background = `${riskColor}06`;
      }}
    >
      <div
        style={{
      ...
      {impacts.length > 0 && (
        <div
          style={{ display: "flex", gap: 6, flexWrap: "wrap", paddingLeft: 30 }}
        >
          {impacts.map(([key, val]) => {
            ...
            return (
              <span
                ...
                style={{
                  ...
                  padding: "1px 7px",
                  borderRadius: 10,
                  background: `${val > 0 ? C.teal : C.red}15`,
                  color: val > 0 ? C.teal : C.red,
                  fontWeight: 600,
                }}
              >
                {s.icon} {val > 0 ? "+" : ""}
...
const PRESETS = [
  {
    label: "Big client vs. recital night",
    events:
      "Huge plumbing lead called right when my kid's school recital starts tonight. Boss-level week at work, running on 4 hrs sleep.",
    context: "Atlanta dad, busy entrepreneur, try to be present for family",
  },
  {
    label: "Launch week chaos",
    events:
      "Product launch Monday, team conflict Tuesday, unexpected expense Thursday, gaming tournament Saturday.",
    context: "Marketing business owner, first big launch of the year",
  },
  {
    label: "The pivot moment",
    events:
      "Top client wants to end contract. New opportunity in a different industry appeared. Savings low but energy high.",
    context: "Trying to decide whether to pivot business direction",
  },
  {
    label: "Family weekend override",
    events:
      "Promised family vacation but three urgent client emails arrived Friday night. Kids excited for beach, phone won't stop buzzing.",
    context: "Work-life balance struggle, trying to honor commitments",
  },
];
...
export default function NarrativeConflictEngine({ onBack }) {
  ...
  const buildStory = useCallback(async () => {
    ...
    try {
      ...
      if (!parsed) {
        setBuildError("Story generation failed — try rephrasing your events.");
        setBuilding(false);
        return;
      }
      ...
      setEventLog([]);
      setCurrentScene({
        scene: parsed.openingScene,
        choice: parsed.choice,
        insightUnlocked: null,
      });
      setIsEnded(false);
      setEnding(null);
  ...
  const makeChoice = useCallback(
    async (option) => {
      ...
      Object.entries(option.statImpacts || {}).forEach(([k, v]) => {
      ...
      const logEntry = {
        ...
        chosenOption: `${option.id}: ${option.label}`,
        insightUnlocked: currentScene.insightUnlocked,
      };
      setEventLog((prev) => [...prev, logEntry]);

      // Build history summary for AI
      const histSummary = [...eventLog, logEntry]
        .map((e, i) => `Beat ${i + 1}: ${e.chosenOption || "opening"}`)
        .join(" → ");
      ...
      const prompt = [
        ...
        `Current stats: ${Object.entries(newStats)
          .map(([k, v]) => `${k}:${v}`)
          .join(", ")}`,
      ...
      Object.entries(parsed.statChanges || {}).forEach(([k, v]) => {
        finalStats[k] = Math.max(0, Math.min(100, (finalStats[k] || 50) + v));
      });
      setStats(finalStats);
      ...
      if (parsed.isEnding) {
        ...
        setEventLog((prev) => [
          ...prev,
          {
            scene: parsed.scene,
            chosenOption: null,
            insightUnlocked: parsed.insightUnlocked,
          },
        ]);
  ...
  function saveStory() {
    ...
    const entry = {
      ...
      genre: story.genre,
      choiceCount: eventLog.length,
      endingType: ending?.type || "In Progress",
      savedAt: new Date().toISOString(),
      storyData: { story, eventLog, stats, ending },
    };
    const next = [entry, ...savedStories].slice(0, 15);
    setSavedStories(next);
    save(STORIES_KEY, next);
  }
  ...
  const generateOutput = useCallback(async () => {
    ...
    setOutputResult(null);
    const storyBeats = eventLog
      .map(
        (e, i) =>
          `Beat ${i + 1}: ${e.chosenOption || "Opening"}\n${(e.scene || "").slice(0, 200)}...`,
      )
      .join("\n\n");
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: `linear-gradient(135deg, ${C.red}15, ${C.orange}10)`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd" }}>
              Your week becomes an interactive life RPG with branching choices
              and real stakes
            </div>
          </div>
          {story && (
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              {(isEnded || eventLog.length > 1) && (
                <button onClick={saveStory} style={btnS(C.gold)}>
                  Save Story
                </button>
              )}
            </div>
        ...
        {TABS.map((t) => (
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              whiteSpace: "nowrap",
              background: activeTab === t.id ? `${C.red}20` : "transparent",
              color: activeTab === t.id ? C.red : "#6aaedd",
              borderBottom:
                activeTab === t.id
                  ? `2px solid ${C.red}`
      ...
      {activeTab === "new" && (
        <div style={{ flex: 1, overflowY: "auto", padding: 24 }}>
          <div style={{ maxWidth: 600 }}>
            ...
            >
              Start a New Life RPG Story
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 20 }}>
              Feed in your real events — the AI turns them into an interactive
              RPG with stats, branching choices, and narrative output you can
              share.
            </div>
              ...
              >
                {PRESETS.map((p) => (
                  <button
                    ...
                    onClick={() => {
                      setEvents(p.events);
                      setContext((c) => ({ ...c, background: p.context }));
                    }}
                    style={{
                      padding: "8px 12px",
                      borderRadius: 10,
                      border: `1px solid ${events === p.events ? C.red : "rgba(255,255,255,0.08)"}`,
                      background:
                        events === p.events ? `${C.red}12` : "transparent",
                      color: events === p.events ? C.red : "#8892a4",
                      cursor: "pointer",
                      textAlign: "left",
                      fontSize: 11,
                    }}
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setContext((c) => ({ ...c, background: e.target.value }))
                    }
                    placeholder="e.g. Atlanta dad, 10yr marketing vet, former gamer"
                    style={inp}
                  />
                </div>
                <div>
                  ...
                  >
                    Invited Characters (optional)
                  </label>
                  <input
                    value={context.invited}
                    onChange={(e) =>
                      setContext((c) => ({ ...c, invited: e.target.value }))
                    }
                    placeholder="e.g. Sarah (spouse), Marcus (business partner)"
                    style={inp}
                  />
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {["humorous", "balanced", "dramatic", "epic"].map((t) => (
                    <button
                      key={t}
                      onClick={() => setContext((c) => ({ ...c, tone: t }))}
                      style={{
                        flex: 1,
                        padding: "6px",
                        borderRadius: 8,
                        border: `1px solid ${context.tone === t ? C.orange : "rgba(255,255,255,0.08)"}`,
                        background:
                          context.tone === t ? `${C.orange}15` : "transparent",
                        color: context.tone === t ? C.orange : "#8892a4",
                        fontSize: 11,
                        cursor: "pointer",
                        fontWeight: context.tone === t ? 700 : 400,
            ...
            <button
              ...
              style={{
                ...
                fontSize: 13,
                fontWeight: 800,
                cursor: events.trim() && !building ? "pointer" : "not-allowed",
                opacity: events.trim() ? 1 : 0.4,
              }}
            >
            ...
            {!story ? (
              ...
              >
                ...
                <div style={{ fontSize: 13 }}>
                  No active story. Create one in the New Story tab.
                </div>
                <button
              ...
              <>
                ...
                >
                  ...
                  >
                    Beat {eventLog.length + (isEnded ? 0 : 1)}
                  </span>
                </div>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
                  ...
                  {eventLog.length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      {eventLog.map((e, i) => (
                        <EventEntry key={i} entry={e} index={i} />
                      ))}
                    </div>
                  )}
            ...
            >
              ...
              {STATS.map((s) => (
                <StatBar
                  key={s.key}
                  stat={s}
                  value={stats[s.key] || 50}
                  delta={statDeltas[s.key] || 0}
                />
              ))}
              ...
              >
                ...
                >
                  Insights from this story are automatically sent to your Life
                  Audit panel.
                </div>
              </div>
              ...
              {pullGameContext() && (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 10, color: "#6aaedd", lineHeight: 1.5 }}
                  >
                    Gaming sessions loaded into story context.
                  </div>
                </div>
              )}
            </div>
      ...
      {activeTab === "archive" && (
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
            Your completed and in-progress life RPG stories
          </div>
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <span style={{ color: endColor, fontWeight: 700 }}>
                          {s.endingType}
                        </span>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button
                        onClick={() => {
                          const next = savedStories.filter(
                            (x) => x.id !== s.id,
                          );
                          setSavedStories(next);
                          save(STORIES_KEY, next);
      ...
      {activeTab === "output" && (
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
            Transform your life RPG story into shareable real-world content
          </div>
          ...
          {!story || eventLog.length < 2 ? (
            <div style={{ textAlign: "center", padding: 40, color: "#4a5568" }}>
              ...
              <div style={{ lineHeight: 1.6 }}>
                Play through at least 2 story beats to unlock output generation.
              </div>
            </div>
          ) : !outputResult ? (
            <div style={{ textAlign: "center", padding: 40 }}>
              ...
              >
                Turn "{story?.storyTitle}" into a blog post, social caption,
                LinkedIn story, video script, or game pitch.
              </div>
              <button
                ...
                style={{
                  ...
                  fontSize: 13,
                  fontWeight: 800,
                  cursor: outputLoading ? "not-allowed" : "pointer",
                }}
              >
                {outputLoading
                  ? "Generating outputs..."
                  : "✂️ Generate All Outputs"}
              </button>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                outputResult[key] ? (
                  ...
                  >
                    ...
                    >
                      <span
                        style={{ fontSize: 11, color: col, fontWeight: 700 }}
                      >
                        {label}
                      </span>
                      <button
                        onClick={() => copy(outputResult[key], key)}
              ...
              <button
                ...
                style={{ ...btnS(C.red), alignSelf: "flex-start" }}
              >
                Regenerate
              </button>
            </div>
