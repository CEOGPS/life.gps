...
export default function AiMoneyTips() {
  const [tips, setTips] = usePersistentState<string[]>("dashboard_money_tips", []);
    ...
    try {
      ...
      const { text } = await lifeosApi.post("/api/llm/invoke", {
        prompt: `Give 3 short, actionable money tips (each under 20 words) for a business owner. ${
          bills.length ? `They currently track ${bills.length} recurring bills.` : "No bill data yet."
  ...
  const display = tips.length ? tips : ["Click Refresh Tips to generate personalized money tips..."];
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
        ...
        {display.map((tip, i) => (
          <div key={i} className="glass rounded-lg p-3 border border-white/5">
            <div className="flex items-start gap-2">
              ...
              <div className={`text-[11px] ${tips.length ? "text-white/70" : "text-white/25 italic"}`}>{tip}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      >
        {loading ? <Loader2 size={11} className="animate-spin" /> : <RefreshCw size={11} />} REFRESH TIPS
      </button>
    </div>
  );
}
