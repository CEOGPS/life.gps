...
export default function LeadsModule() {
  ...
  const visible = leads.filter((l) => l.name.toLowerCase().includes(filter.toLowerCase()));

  const addLead = () => {
    if (!name.trim()) return;
    setLeads([{ id: crypto.randomUUID(), name: name.trim(), source, addedAt: new Date().toISOString() }, ...leads]);
    setName("");
    setAdding(false);
  };
    ...
    <div className="flex flex-col gap-3 h-full">
      ...
      {adding && (
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={addLead} className="text-[10px] text-primary/70 hover:text-primary">
            Save
          </button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto">
        {visible.length === 0 ? (
          ...
          <div className="flex flex-col gap-1">
            {visible.map((l) => (
              <div key={l.id} className="flex items-center justify-between px-2 py-1.5 rounded bg-white/4 border border-white/6">
                <div>
      ...
      <div className="flex gap-1.5 flex-wrap border-t border-white/5 pt-2">
        {SOURCES.map((s) => (
          <button
            key={s}
            onClick={() => setFilter(filter === s ? "" : "")}
            className="text-[9px] px-2 py-0.5 rounded-full border border-white/8 text-white/25 hover:text-white/50 hover:border-white/20"
          >
            {s}
          </button>
        ))}
