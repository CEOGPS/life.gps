...
import { GENRES } from '@/pages/veriton/lib/generationStore.js';
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-display font-bold crimson-text-gradient">Library</h1>
        <button
      ...
      <div className="glass p-4 mb-6 flex items-center gap-3" style={{ borderRadius: 16 }}>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="glass p-3 h-52 animate-pulse" style={{ borderRadius: 18 }} />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filtered.map((track, i) => (
            <TiltCard key={track.id} className="p-3 group" onClick={() => navigate(`/track/${track.id}`)}>
              <div className="relative aspect-square rounded-xl mb-2 overflow-hidden flex items-center justify-center" style={{ background: `linear-gradient(135deg, hsl(${(i * 40) % 360}, 70%, 30%), #050505)` }}>
                <span className="text-white/30 font-display font-bold text-4xl">{track.title?.[0]?.toUpperCase()}</span>
                <button
