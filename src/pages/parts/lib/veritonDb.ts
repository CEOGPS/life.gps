...
function makeEntity(entityName: string) {
  ...
  if (!table) {
    console.warn(`[veritonDb] Unknown entity "${entityName}" — no backing table configured`);
  }
  ...
  return {
    ...
    async filter(query: Record<string, unknown>) {
      ...
      if (error) {
        console.error(`[veritonDb] ${entityName}.filter failed:`, error.message);
        return [];
      }
      return data ?? [];
    },
    ...
    async create(fields: Record<string, unknown>) {
      const { data, error } = await supabase.from(table).insert(fields).select().maybeSingle();
      if (error) {
        console.error(`[veritonDb] ${entityName}.create failed:`, error.message);
        throw error;
      }
      return data;
    },
    ...
    async update(id: string, fields: Record<string, unknown>) {
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) {
        console.error(`[veritonDb] ${entityName}.update failed:`, error.message);
        throw error;
      }
      return data;
    },
    ...
    async delete(id: string) {
      ...
      if (error) {
        console.error(`[veritonDb] ${entityName}.delete failed:`, error.message);
        throw error;
      }
      return true;
    },
...
export const db = {
  entities: new Proxy(
    {},
    {
      get: (_target, prop: string) => makeEntity(prop),
    }
  ),
  integrations: {
    Core: {
      async UploadFile({ file }: { file: File }) {
        ...
        const { error } = await supabase.storage.from("veriton-uploads").upload(path, file);
        if (error) {
          console.error("[veritonDb] UploadFile failed:", error.message);
          return { file_url: "" };
        }
        const { data } = supabase.storage.from("veriton-uploads").getPublicUrl(path);
        return { file_url: data.publicUrl };
      },
      async InvokeLLM() {
        // Generation backend not wired yet (Base44's was proprietary) — stubbed per plan.
        console.warn("[veritonDb] InvokeLLM called — no generation backend configured yet.");
