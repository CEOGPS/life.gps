      ...
      <div className="h-full flex gap-3 overflow-y-auto">
        ...
        <div className="w-44 shrink-0 flex flex-col gap-3">
          <div className="glass rounded-xl border border-white/8 p-2 flex-1 flex flex-col">
            <div
              className="text-[9px] font-display tracking-widest px-2 mb-2"
              style={{ color: TEAL }}
            >
              CHANNELS
            </div>
            <div className="space-y-0.5 flex-1">
              {CHANNELS.map((c) => (
                ...
                >
                  ...
                  <span
                    className={`text-[11px] flex-1 ${CHANNEL_COLORS[c.name] ?? "text-white/55"}`}
                  >
                    {c.name}
                  </span>
              ...
              <div className="h-full flex items-center justify-center p-4">
                <div className="text-center">
                  ...
                  <div className="text-xs text-white/55">
                    Connect channels to see your unified message feed
                  </div>
                </div>
              </div>
            ) : (
                ...
                {FEED_MESSAGES.map((msg) => (
                  ...
                  >
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span
                        className={`text-[9px] font-display px-1.5 py-0.5 rounded-full glass-crimson text-primary`}
                      >
                        {msg.channel}
                      </span>
                    ...
                    <div className="flex gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-0.5 text-white/30 hover:text-white/70">
                        <Bookmark size={9} />
                      </button>
                      <button className="p-0.5 text-white/30 hover:text-primary">
                        <Trash2 size={9} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="flex-1 flex flex-col glass rounded-xl border border-white/8 overflow-hidden min-w-0">
          ...
          <div className="p-3 border-b border-white/5 flex items-center gap-2 shrink-0">
            ...
            <div className="flex gap-1">
              <button className="text-white/30 hover:text-white/70 p-1.5 rounded transition-colors">
                <Phone size={13} />
              </button>
              <button className="text-white/30 hover:text-white/70 p-1.5 rounded transition-colors">
                <Video size={13} />
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
            {messageList.length === 0 ? (
              ...
              messageList.map((m) => (
                ...
                >
                  <div
                    className={`max-w-[70%] px-3 py-2 rounded-xl text-xs ${m.from === "me" ? "glass-crimson text-white/80" : "glass text-white/70"}`}
                  >
                    {m.text}
                    <div className="text-[9px] text-white/30 mt-0.5 text-right">
                      {m.time}
                    </div>
                  </div>
                </div>
