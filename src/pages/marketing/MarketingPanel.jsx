...
const DEFAULT_LISTINGS = [
  ...
  },
  {
    name: "HomeAdvisor",
    icon: "🔧",
    url: "https://www.homeadvisor.com/business-center",
    color: C.teal,
    priority: "medium",
  },
  {
    name: "BBB",
    icon: "🏛️",
    url: "https://www.bbb.org/accreditation",
    color: C.blue,
    priority: "medium",
  },
  {
...
function load(key, fallback) {
  try {
    return (
      JSON.parse(localStorage.getItem("lifeos_mktg_" + key) || "null") ??
      fallback
    );
  } catch {
    return fallback;
...
function save(key, val) {
  try {
    localStorage.setItem("lifeos_mktg_" + key, JSON.stringify(val));
  } catch {}
}
...
function ListingsTab() {
  ...
  useEffect(() => {
    save("listing_statuses", listingStatuses);
  }, [listingStatuses]);
  useEffect(() => {
    if (bizInfo) save("biz_info", bizInfo);
  }, [bizInfo]);
  useEffect(() => {
    if (oppResults) save("opp_results", oppResults);
  }, [oppResults]);
  useEffect(() => {
    if (scanResults) save("scan_results", scanResults);
  }, [scanResults]);
  ...
  const activeCount = Object.values(listingStatuses).filter(
    (s) => s === "Claimed" || s === "Active",
  ).length;
...
No markdown, no explanation — raw JSON only.`;
    ...
    try {
      const raw = await invokeLLM({
        prompt,
        systemPrompt:
          "You are a local SEO and citation building expert. Return only valid JSON arrays.",
      });
      const clean = raw.replace(/```json|```/g, "").trim();
      const data = JSON.parse(clean);
      setOppResults({
        items: data,
        company: oppInput.company,
...
No markdown, raw JSON only.`;
    ...
    try {
      const raw = await invokeLLM({
        prompt,
        systemPrompt:
          "You are a citation consistency auditor. Return only valid JSON.",
      });
      const clean = raw.replace(/```json|```/g, "").trim();
      const data = JSON.parse(clean);
      ...
      setScanResults({
        error: "Scan failed. Check your business info and try again.",
        issues: [],
      });
    }
  ...
  const SUB_TABS = [
    {
      id: "dashboard",
      label: "📋 Dashboard",
      count: activeCount + "/" + DEFAULT_LISTINGS.length,
    },
    {
      id: "bizinfo",
      label: "🏢 Business Info",
      badge: !hasBizInfo ? "!" : null,
    },
        ...
        {SUB_TABS.map((t) => (
          <button
            ...
            style={{
              ...
              border: "0.5px solid",
              background:
                subTab === t.id ? "rgba(74,179,244,0.15)" : "transparent",
              borderColor: subTab === t.id ? C.blue : "rgba(255,255,255,0.12)",
              color: subTab === t.id ? C.blue : "#6aaedd",
              display: "flex",
              alignItems: "center",
              gap: 6,
            }}
        ...
        <>
          ...
          >
            {[
              ...
              {
                label: "Needs Work",
                val: Object.values(listingStatuses).filter(
                  (s) => s === "Needs Update",
                ).length,
                color: C.orange,
              },
              {
                label: "Unclaimed",
                val:
                  DEFAULT_LISTINGS.length -
                  Object.keys(listingStatuses).filter(
                    (k) =>
                      listingStatuses[k] === "Claimed" ||
                      listingStatuses[k] === "Active",
                  ).length,
                color: C.red,
              },
              {
                label: "Total Tracked",
                val: DEFAULT_LISTINGS.length,
                color: C.blue,
              },
            ].map((s) => (
              <div
                  ...
                  {isEditing ? (
                    <div>
                      <select
                        value={status}
                        onChange={(e) =>
                          setListingStatus(l.name, e.target.value)
                        }
                        style={{ ...inp, fontSize: 11, marginBottom: 6 }}
                      >
                        {["Unclaimed", "Claimed", "Active", "Needs Update"].map(
                          (s) => (
                      ...
                      <button
                        onClick={() => setListingEditing(null)}
                        style={{
                          width: "100%",
                          padding: "5px",
                          borderRadius: 6,
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button
                        onClick={() => setListingEditing(l.name)}
                        style={{
                          flex: 1,
                          padding: "6px",
                          borderRadius: 7,
        ...
        <div style={{ ...card, padding: 22 }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 11, color: "#6aaedd", marginTop: 2 }}>
                Your canonical source of truth — used by the scanner, finder &
                sync push.
              </div>
            </div>
            {bizSaved && (
          ...
          {!bizEditing && hasBizInfo ? (
            <div>
              ...
              >
                {[
                  ...
                  [
                    "City / State / Zip",
                    `${bizInfo.city}, ${bizInfo.state} ${bizInfo.zip}`,
                  ],
                  ["Hours", bizInfo.hours],
                  ["Email", bizInfo.email],
                ].map(([label, val]) => (
              ...
              {bizInfo.description && (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 12, color: "#c8c8d0", lineHeight: 1.6 }}
                  >
                    {bizInfo.description}
                  </div>
                </div>
            ...
            <div style={{ marginTop: 14 }}>
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input
                      ...
                      placeholder={ph}
                      onChange={(e) =>
                        setBizInfo((b) => ({ ...b, [key]: e.target.value }))
                      }
                      style={inp}
                    />
                  </div>
              ...
              <div style={{ marginTop: 10 }}>
                ...
                >
                  Business Description (150-300 words recommended)
                </div>
                <textarea
                  value={bizInfo.description || ""}
                  onChange={(e) =>
                    setBizInfo((b) => ({ ...b, description: e.target.value }))
                  }
              ...
              <div style={{ marginTop: 10 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBizInfo((b) => ({ ...b, tagline: e.target.value }))
                  }
      ...
      {subTab === "finder" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ ...card, padding: 20 }}>
            ...
            >
              🔍 Listing Opportunity Finder
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 14 }}>
              Enter any business — find every directory they're missing out on.
            </div>
            <div
              style={{
                display: "grid",
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    placeholder={ph}
                    onChange={(e) =>
                      setOppInput((i) => ({ ...i, [key]: e.target.value }))
                    }
            ...
            <button
              ...
              disabled={oppLoading || !oppInput.company.trim()}
              style={{
                marginTop: 14,
                width: "100%",
                padding: "10px",
                ...
                fontWeight: 700,
                cursor: "pointer",
                opacity: oppLoading || !oppInput.company.trim() ? 0.6 : 1,
              }}
            >
              {oppLoading
                ? "🔍 Scanning directory universe..."
            ...
            <div style={{ ...card, padding: 20 }}>
              ...
              >
                <div>
                  ...
                  >
                    Opportunities for "{oppResults.company}"
                  </div>
                  <div style={{ fontSize: 10, color: "#6aaedd", marginTop: 2 }}>
                    Scanned {oppResults.scannedAt} · {oppResults.items?.length}{" "}
                    opportunities found
                  </div>
                </div>
                <button
                ...
                {(oppResults.items || []).map((item, i) => (
                  <div
                    ...
                    style={{
                      ...
                      border: `0.5px solid ${sevColor(item.priority)}33`,
                    }}
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div
                        style={{ fontSize: 10, color: "#6aaedd", marginTop: 1 }}
                      >
                        {item.reason}
                      </div>
                    </div>
                    ...
                    >
                      <span
                        style={{
                          ...
                          padding: "2px 7px",
                          borderRadius: 20,
                          background: sevColor(item.priority) + "22",
                          color: sevColor(item.priority),
                          fontWeight: 700,
                        }}
                      >
                        {item.priority}
          ...
          {oppResults?.error && (
            <div
              style={{ ...card, padding: 16, border: `0.5px solid ${C.red}44` }}
            >
      ...
      {subTab === "scanner" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {!hasBizInfo && (
            ...
            >
              <div style={{ fontSize: 12, color: C.orange, fontWeight: 600 }}>
                ⚠ Set your Business Info first
              </div>
              <div style={{ fontSize: 11, color: "#c8c8d0", marginTop: 4 }}>
                The scanner needs your canonical info to check against.{" "}
                <button
                  onClick={() => setSubTab("bizinfo")}
                  style={{
                    color: C.blue,
                    background: "none",
                    border: "none",
          ...
          <div style={{ ...card, padding: 20 }}>
            ...
            >
              🩺 Listing Inconsistency Scanner
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 14 }}>
              Audits your business info for NAP inconsistency risks, incomplete
              fields, and data quality issues across directories.
            </div>
            {hasBizInfo && (
              <div
                style={{
              ...
              >
                ...
                · {bizInfo.address}, {bizInfo.city} · {bizInfo.phone}
              </div>
            )}
            <button
              onClick={runInconsistencyScan}
              disabled={!hasBizInfo || scanLoading}
              style={{
                width: "100%",
                padding: "10px",
                borderRadius: 10,
                background: hasBizInfo
                  ? `linear-gradient(135deg,${C.purple},${C.blue})`
                  : "rgba(255,255,255,0.06)",
                border: "none",
                color: hasBizInfo ? "#000" : "#444",
                fontSize: 13,
                fontWeight: 700,
                cursor: hasBizInfo ? "pointer" : "not-allowed",
              }}
            >
              {scanLoading
                ? "🩺 Auditing your listing data..."
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    Issues Found: {scanResults.issues?.length || 0}
                  </div>
                  <div
                    style={{
                      height: 6,
                  ...
                  >
                    <div
                      style={{
                        width: `${scanResults.score}%`,
                        height: "100%",
                        borderRadius: 3,
                        background: `linear-gradient(90deg,${scanResults.score >= 80 ? C.teal : scanResults.score >= 60 ? C.amber : C.red},${C.blue})`,
                        transition: "width .5s",
                      }}
                    />
                  </div>
                    ...
                    {scanResults.issues.map((issue, i) => (
                      ...
                      >
                        ...
                        <div style={{ fontSize: 11, color: C.teal }}>
                          <span style={{ color: "#6aaedd" }}>Fix: </span>
                          {issue.fix}
                        </div>
                      </div>
                    ))}
              ...
              {scanResults.napRisks?.length > 0 && (
                <div style={{ ...card, padding: 16 }}>
                  ...
                  >
                    These name/address variations may exist across directories —
                    search each to find and correct them:
                  </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {!hasBizInfo && (
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "#c8c8d0", marginTop: 4 }}>
                The sync tool uses your canonical info to generate update
                guides.{" "}
                <button
                  onClick={() => setSubTab("bizinfo")}
                  style={{
          ...
          <div style={{ ...card, padding: 20 }}>
            ...
            >
              🔄 Sync & Push to All Listings
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 6 }}>
              Your canonical info gets pushed to each listing with AI-generated
...
export default function MarketingPanel() {
  ...
  function logActivity(icon, label, color) {
    ...
    setActivityLog((prev) => {
      ...
      save("activity_log", updated);
      return updated;
    });
  }

  useEffect(() => {
    save("seo_checks", localChecks);
  }, [localChecks]);
  useEffect(() => {
    save("seo_target", seoTarget);
  }, [seoTarget]);
  useEffect(() => {
  ...
  async function doKeyword() {
    ...
    setSeoLoadingKey("keyword");
    setKeyResult("");
    const r = await invokeLLM(
      `You are an SEO expert. Research keywords related to: "${keyword}" for ${bizCtx}. Provide 10 keyword suggestions with estimated monthly search volume, keyword difficulty (Low/Med/High), and search intent (Informational/Commercial/Transactional). Format as a clean numbered list.`,
    );
    setKeyResult(r);
    setSeoLoadingKey(null);
    logActivity("🔍", `Keyword research: "${keyword}"`, C.blue);
  }

  async function doMeta() {
    if (!metaUrl.trim() || seoLoadingKey) return;
    setSeoLoadingKey("meta");
    setMetaResult("");
    const r = await invokeLLM(
      `Generate an SEO-optimized meta title (max 60 chars) and meta description (max 155 chars) for ${bizCtx}. Page/topic: "${metaUrl}". Provide both clearly labeled.`,
    );
    setMetaResult(r);
    setSeoLoadingKey(null);
  ...
  async function doAudit() {
    ...
    setSeoLoadingKey("audit");
    setSeoAudit("");
    const r =
      await invokeLLM(`Perform a comprehensive SEO audit for ${bizCtx}. Cover:
  ...
  const overviewStats = [
    {
      ...
      val: campaigns.filter((c) => c.status === "Active").length || "0",
      color: C.blue,
      icon: "📢",
    },
    {
    ...
    },
    {
      ...
      val: `${Object.values(listingStatuses).filter((s) => s === "Claimed" || s === "Active").length}/${DEFAULT_LISTINGS.length}`,
      color: C.teal,
      icon: "📍",
    },
    {
      label: "SEO Checklist",
      val: `${localChecks.filter(Boolean).length}/${LOCAL_SEO_ITEMS.length}`,
      color: C.orange,
      icon: "✅",
    },
  ];
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 24 }}>
          ...
          <>
            ...
            <div style={{ ...card, padding: 20 }}>
              ...
              {activityLog.length === 0 ? (
                ...
                >
                  No activity yet — start using Marketing tools to see your log
                  here.
                </div>
              ) : (
                activityLog.slice(0, 8).map((a, i) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, fontSize: 12, color: "#c8c8d0" }}>
                      {a.label}
                    </div>
        ...
        {tab === "SEO Tools" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            >
              ...
              >
                🎯 Target Company — all tools below run against this
              </div>
              <div
                style={{
                  display: "grid",
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input
                      ...
                      placeholder={ph}
                      onChange={(e) =>
                        setSeoTarget((t) => ({ ...t, [key]: e.target.value }))
                      }
                ...
                <div style={{ marginTop: 8, fontSize: 11, color: "#6aaedd" }}>
                  ...
                  {seoTarget.website && (
                    <>
                      {" "}
                      ·{" "}
                      <span style={{ color: C.teal }}>{seoTarget.website}</span>
                    </>
                  )}
            ...
            <div style={{ ...card, padding: 18 }}>
              ...
              <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 12 }}>
                {seoTarget.company
                  ? `Running against: ${seoTarget.company}`
                  : "Set a target company above first"}
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[
                  ...
                  <button
                    ...
                    onClick={btn.fn}
                    disabled={
                      !seoTarget.company.trim() || seoLoadingKey === btn.key
                    }
                    style={{
                      padding: "9px 18px",
                      borderRadius: 8,
                      background: `${btn.color}20`,
                      border: `0.5px solid ${btn.color}55`,
                      color: btn.color,
                      fontSize: 12,
                      fontWeight: 700,
                      cursor: seoTarget.company ? "pointer" : "not-allowed",
                      opacity: seoTarget.company ? 1 : 0.5,
                    }}
                  >
                    {seoLoadingKey === btn.key
                      ? "◈ Running..."
                      : `${btn.icon} ${btn.label}`}
                  </button>
                ))}
              ...
              {!seoTarget.company && (
                ...
                >
                  ← Enter a company name in the Target bar to unlock these tools
                </div>
              )}
              {seoAudit && (
                <div
            ...
            <div style={{ ...card, padding: 18 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  placeholder="Seed keyword or topic (e.g. 'digital marketing Atlanta')"
                  style={inp}
                />
                <button
                  onClick={doKeyword}
                  disabled={!!seoLoadingKey || !keyword.trim()}
                  style={{
                    padding: "9px 18px",
                    borderRadius: 8,
            ...
            <div style={{ ...card, padding: 18 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  placeholder="Page URL or topic (e.g. 'homepage', 'services page', 'about us')"
                  style={inp}
                />
                <button
                  onClick={doMeta}
                  disabled={!!seoLoadingKey || !metaUrl.trim()}
                  style={{
                    padding: "9px 18px",
                    borderRadius: 8,
              ...
              {seoTarget.company && (
                <div style={{ fontSize: 10, color: "#444", marginTop: 4 }}>
                  Context: {seoTarget.company}
                  {seoTarget.industry ? ` · ${seoTarget.industry}` : ""}
                </div>
              )}
              {metaResult && (
                <div
            ...
            <div style={{ ...card, padding: 18 }}>
              ...
              <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 10 }}>
                {localChecks.filter(Boolean).length}/{LOCAL_SEO_ITEMS.length}{" "}
                complete
                {seoTarget.company && (
                  <span style={{ marginLeft: 8, color: "#444" }}>
                    — tracking for {seoTarget.company}
                  </span>
                )}
              </div>
              <div
              ...
              >
                <div
                  style={{
                    width: `${(localChecks.filter(Boolean).length / LOCAL_SEO_ITEMS.length) * 100}%`,
                    height: "100%",
              ...
              {LOCAL_SEO_ITEMS.map((item, i) => (
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 12,
                      color: localChecks[i] ? C.teal : "#c8c8d0",
                      textDecoration: localChecks[i] ? "line-through" : "none",
                    }}
                  >
                    {item}
                  </span>
          ...
          <>
              ...
              <div style={{ ...card, overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    ...
                    >
                      {["Campaign", "Subject", "Status", "Created", ""].map(
                        (h) => (
                          <th
                            key={h}
                            style={{
            ...
            {newCampaignOpen && (
              <div
                ...
                onClick={(e) => {
                  if (e.target === e.currentTarget) setNewCampaignOpen(false);
                }}
              >
                <div
                  style={{
                ...
                >
                  ...
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <select
                      ...
                      style={inp}
                    >
                      {["Draft", "Active", "Sent", "Paused"].map((s) => (
                        <option key={s}>{s}</option>
                      ))}
                    </select>
                  </div>
        ...
        {tab === "Search Analytics" && (
          <div style={{ ...card, padding: 40, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 12, color: "#6aaedd", marginBottom: 16 }}>
              Link Google Search Console and Analytics for real data here.
            </div>
...
function PeopleCapitalVault() {
  ...
  const [overrides, setOverrides] = useState(() => {
    try {
      return JSON.parse(
        localStorage.getItem("lifeos_capital_overrides") || "{}",
      );
    } catch {
      return {};
    }
  ...
  useEffect(() => {
    const contacts = (() => {
      try {
        return JSON.parse(localStorage.getItem("lifeos_contacts") || "[]");
      } catch {
        return [];
      }
    })();
    ...
    const family = (() => {
      try {
        return JSON.parse(localStorage.getItem("family_members_v2") || "[]");
      } catch {
        return [];
      }
    })();
    ...
    const merged = [
      ...
      ...crm.map((c) => ({
        ...
        phone: c.phone || "",
        lastContact: c.lastContact || null,
        tags: [c.tag, c.stage].filter(Boolean),
        notes: c.notes || "",
        birthday: c.birthday || "",
        company: c.company || "",
        value: c.value || "",
    ...
    const scored = merged.map((p) => {
      ...
      if (override !== undefined)
        return { ...p, capitalScore: override, scoreReason: "Manual override" };
      ...
      if (p.type === "Family") {
        score += 15;
        reasons.push("Family connection");
      }
      if (p.type === "Business") {
        score += 5;
        reasons.push("Business contact");
      }

      // Tags
      if (p.tags.includes("VIP")) {
        score += 20;
        reasons.push("VIP tag");
      }
      if (p.tags.includes("Hot")) {
        score += 18;
        reasons.push("Hot lead");
      }
      if (p.tags.includes("Warm")) {
        score += 10;
        reasons.push("Warm lead");
      }
      if (p.tags.includes("Cold")) {
        score -= 15;
        reasons.push("Cold tag");
      }
      if (p.tags.includes("Referral")) {
        score += 12;
      ...
      } else if (lc.includes("Just now") || lc.includes("Today")) {
        score += 20;
        reasons.push("Contacted today");
      } else if (lc.includes("day") && parseInt(lc) < 3) {
        score += 15;
        reasons.push("Contacted recently");
      } else if (lc.includes("week") && parseInt(lc) < 2) {
        score += 8;
        reasons.push("Contacted this week");
      } else if (lc.includes("month") && parseInt(lc) < 2) {
        score += 2;
      } else if (lc.includes("month") || lc.includes("ago")) {
        score -= 10;
        reasons.push("Overdue outreach");
      }
      ...
      if (p.birthday) {
        score += 5;
        reasons.push("Birthday tracked");
      }

      return {
        ...p,
        capitalScore: Math.max(0, Math.min(100, Math.round(score))),
        scoreReason: reasons.slice(0, 3).join(" · "),
      };
    });
...
Sound natural, warm, brief (30-45 seconds when spoken). Mention something personal, then a light business touch if appropriate.`,
    ...
    const res = await invokeLLM({
      prompt: nudgePrompts[type] || nudgePrompts.message,
    });
    setNudgeResult(res);
  ...
  const stats = {
    ...
    warm: people.filter((p) => p.capitalScore >= 45 && p.capitalScore < 80)
      .length,
    cold: people.filter((p) => p.capitalScore < 45).length,
    overdue: people.filter((p) => {
      const lc = p.lastContact || "";
      return !lc || lc === "Never" || lc.includes("month");
    }).length,
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[
            ["🔥", stats.hot, "Hot", "#ff4f5e"],
            ["⚡", stats.warm, "Warm", "#ff8c42"],
            ["❄", stats.cold, "Cold", "#4ab3f4"],
            ["⏰", stats.overdue, "Overdue", "#8b7fff"],
        ...
        >
          {filtered.length === 0 && (
            ...
            >
              No contacts loaded yet — add people in Contacts, CRM, or Family
              Hub.
            </div>
          )}
          {filtered.map((p) => {
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div
                      style={{ fontSize: 16, fontWeight: 700, color: sl.color }}
                    >
                      {p.capitalScore}
                    </div>
                    <div
                      style={{ fontSize: 8, color: sl.color, fontWeight: 600 }}
                    >
                      {sl.label}
                    </div>
                  </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {!selected && (
          <div
            style={{ ...card, padding: 30, textAlign: "center", color: "#444" }}
          >
            <div style={{ fontSize: 32, marginBottom: 12 }}>🏛</div>
            <div
              style={{
            ...
            >
              AI-powered relationship health scores across every connection —
              family, friends, clients, and prospects. Select anyone to generate
              personalized nurture nudges.
            </div>
            <div
              style={{
                marginTop: 20,
            ...
            >
              {[
                ["🏛", "Total", people.length, "#4ab3f4"],
                ["🔥", "Hot Relationships", stats.hot, "#ff4f5e"],
                ["⏰", "Need Nurturing", stats.overdue, "#8b7fff"],
          ...
          <div>
            ...
            <div style={{ ...card, padding: 20, marginBottom: 14 }}>
                ...
                <div style={{ textAlign: "center", flexShrink: 0 }}>
                  <div
                    style={{
                      fontSize: 36,
                      fontWeight: 700,
                      color: scoreLabel(selected.capitalScore).color,
                      lineHeight: 1,
                    }}
                  >
                    {editingScore === selected.id ? (
                      <input
                        ...
                        min={0}
                        max={100}
                        defaultValue={selected.capitalScore}
                        onBlur={(e) =>
                          saveOverride(selected.id, e.target.value)
                        }
                        onKeyDown={(e) =>
                          e.key === "Enter" &&
                          saveOverride(selected.id, e.target.value)
                  ...
                  >
                    {scoreLabel(selected.capitalScore).label}
                  </div>
                  ...
                  <button
                    onClick={() =>
                      setEditingScore(
                        editingScore === selected.id ? null : selected.id,
                      )
                    }
                    style={{
                      fontSize: 9,
              ...
              >
                {[
                  ...
                  .filter(([v]) => v)
                  .map(([val, icon]) => (
                    <span key={icon} style={{ fontSize: 11, color: "#888" }}>
                      {icon} {val}
                    </span>
                  ))}
              </div>
            ...
            <div style={{ ...card, padding: 20, marginBottom: 14 }}>
              ...
              <div style={{ fontSize: 11, color: "#555", marginBottom: 14 }}>
                Generate a personalized outreach for{" "}
                {selected.name.split(" ")[0]} — the AI uses their profile, tags,
                birthday, and your relationship history.
              </div>
              ...
              >
                {[
                  ...
                  <button
                    key={type}
                    onClick={() => generateNudge(selected, type)}
                    ...
                    style={{
                      ...
                      fontSize: 11,
                      fontWeight: 600,
                      cursor: nudgeLoading ? "wait" : "pointer",
                      background:
                        nudgeType === type && nudgeLoading
                          ? `${color}30`
                          : `${color}15`,
              ...
              {nudgeResult && (
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                    <button
                      onClick={() =>
                        navigator.clipboard?.writeText(nudgeResult)
                      }
                      style={{
                        fontSize: 11,
                        padding: "5px 14px",
                    ...
                    <button
                      onClick={() => generateNudge(selected, nudgeType)}
                      disabled={nudgeLoading}
                      style={{
                        fontSize: 11,
                        padding: "5px 14px",
            ...
            {selected.kpi && (
              <div style={{ ...card, padding: 16, marginBottom: 14 }}>
                ...
                {[
                  ["Connection", selected.kpi.connect, C.teal],
                  ["Support", selected.kpi.support, C.purple],
                ].map(([label, val, color]) => (
                  <div key={label} style={{ marginBottom: 10 }}>
                    ...
                    >
                      <span>{label}</span>
                      <span style={{ color, fontWeight: 600 }}>{val}/10</span>
                    </div>
                    <div
                      style={{
                        height: 4,
