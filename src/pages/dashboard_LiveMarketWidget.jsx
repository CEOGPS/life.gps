  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {lastUpdate && (
              <div style={{ fontSize: 9, color: "#2a6fa8" }}>
        ...
        {cryptoLoading ? (
          ...
          <div style={{ fontSize: 11, color: "#ff4f5e" }}>
            Unable to reach CoinGecko — check connection
          </div>
        ) : (
          <>
            {/* Scrolling ticker */}
                ...
                {[...COINS, ...COINS].map((coin, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span style={{ color: "#c8c8d0", marginLeft: 4 }}>
                        ${d.usd?.toLocaleString()}
                      </span>
                      <span
                        style={{ color: up ? C.teal : C.red, marginLeft: 4 }}
                      >
                        {up ? "▲" : "▼"}
                        {Math.abs(change)}%
                      </span>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      padding: "8px 10px",
                      border: `0.5px solid ${up ? "rgba(0,200,150,0.2)" : "rgba(255,79,94,0.2)"}`,
                    }}
                  >
                    <div
                      style={{
                    ...
                    <div style={{ fontSize: 9, color: up ? C.teal : C.red }}>
                      {up ? "▲" : "▼"}
                      {Math.abs(change)}%
                    </div>
                  </div>
      ...
      <div style={{ ...card, padding: 14 }}>
            ...
            {NEWS_FEEDS.map((f, i) => (
              <button
                ...
                style={{
                  ...
                  border: "0.5px solid",
                  background:
                    newsFeed === i ? "rgba(255,140,66,0.15)" : "transparent",
                  borderColor:
                    newsFeed === i ? C.orange : "rgba(255,255,255,0.1)",
                  color: newsFeed === i ? C.orange : "#6aaedd",
                }}
              >
                {f.label}
              </button>
        ...
        {newsLoading ? (
          <div style={{ fontSize: 11, color: "#2a6fa8" }}>
            Loading {NEWS_FEEDS[newsFeed].label} news...
          </div>
        ) : news.length === 0 ? (
          <div style={{ fontSize: 11, color: "#ff4f5e" }}>
            Could not load news — RSS may be blocked. Try another feed.
          </div>
        ) : (
          <div
            style={{
          ...
          >
            {news.map((item, i) => (
              ...
              >
                ...
                {item.desc && (
                  <div
                    style={{ fontSize: 10, color: "#6aaedd", lineHeight: 1.4 }}
                  >
                    {item.desc}...
                  </div>
                )}
                {item.pub && (
                  <div style={{ fontSize: 9, color: "#2a6fa8", marginTop: 4 }}>
                    {new Date(item.pub).toLocaleString()}
                  </div>
                )}
              </a>
            ))}
