      ...
      <div className="h-full flex flex-col gap-4">
        <div className="flex gap-1 overflow-x-auto shrink-0">
          {TABS.map((t, i) => (
            <button 
              key={t} 
              onClick={() => setTab(i)} 
              className={`px-3 py-1.5 rounded-lg text-[11px] font-display tracking-wider transition-all whitespace-nowrap
              ${tab === i ? "glass-crimson text-primary glow-crimson-sm" : "glass text-white/30 hover:text-white/60"}`}
            >
              {t.toUpperCase()}
            </button>
          ))}
        ...
        <div className="flex-1 min-h-0 grid grid-cols-3 gap-4">
          {[
            ...
            <div key={tool.title} className="glass rounded-xl p-4 border border-white/10 hover:border-primary/30 cursor-pointer group transition-all hover:glow-crimson">
              ...
              <div className="text-xs text-white/80 font-medium mb-1 group-hover:text-white transition-colors">{tool.title}</div>
        ...
        <div className="grid grid-cols-4 gap-3 shrink-0">
          {["Organic Traffic", "Domain Rating", "Backlinks", "Keywords Tracked"].map((m) => (
            <div key={m} className="glass rounded-xl p-3 border border-white/10 text-center">
              <div className="text-[9px] text-teal font-display tracking-wider uppercase">{m}</div>
