...
export default function MessagesPanel() {
  ...
  async function draftAIReply() {
    ...
    try {
      const history = activeMessages
        ...
        .map(
          (m) =>
            `${m.sender_type === "me" ? "You" : active.contact_name}: ${m.content}`,
        )
        .join("\n");

      const platformLabel =
        PLATFORMS.find((p) => p.id === primaryPlatform)?.label || "text";
  ...
  const filtered = conversations.filter((c) => {
    const matchesSearch = c.contact_name
      .toLowerCase()
      .includes(search.toLowerCase());
    const matchesPlatform =
      filterPlatform === "all" || c.platforms?.includes(filterPlatform);
    return matchesSearch && matchesPlatform;
  });

  if (!user) {
    return (
      <div style={{ padding: 20, color: "#6aaedd" }}>
  ...
  return (
    ...
    >
      ...
      >
          ...
          >
            <button
              ...
              style={{
                ...
                border:
                  "0.5px solid " +
                  (filterPlatform === "all" ? C.blue : "rgba(255,255,255,0.1)"),
                background:
                  filterPlatform === "all"
        ...
        >
          <div style={{ marginBottom: 8, color: "#2a6fa8" }}>
            Sync platforms:
          </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  ...
                  {active.is_group && (
                    <span
                      style={{ fontSize: 10, color: "#2a6fa8", marginLeft: 6 }}
                    >
                      👥 Group
                    </span>
                  )}
              ...
              <button
                onClick={() => setInCall(inCall === "voice" ? null : "voice")}
                style={{
                  padding: "7px 14px",
                  borderRadius: 20,
                  background:
                  ...
                  border:
                    "0.5px solid " +
                    (inCall === "voice" ? C.teal : "rgba(74,179,244,0.3)"),
                  color: inCall === "voice" ? C.teal : C.blue,
                  fontSize: 11,
                  fontWeight: 600,
                  cursor: "pointer",
                }}
              ...
              <button
                onClick={() => setInCall(inCall === "video" ? null : "video")}
                style={{
                  padding: "7px 14px",
                  borderRadius: 20,
                  background:
                  ...
                  border:
                    "0.5px solid " +
                    (inCall === "video" ? C.purple : "rgba(139,127,255,0.3)"),
                  color: C.purple,
                  fontSize: 11,
                  fontWeight: 600,
                  cursor: "pointer",
                ...
                activeMessages.map((m, i) => (
                  <div
                    ...
                    style={{
                      display: "flex",
                      justifyContent:
                        m.sender_type === "me" ? "flex-end" : "flex-start",
                      alignItems: "flex-end",
                      gap: 6,
                    }}
                  >
