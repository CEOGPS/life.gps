...
import { Zap, Upload, Check, Sliders, ChevronDown, Waves, User, Volume2, Mic2, Music2, Gauge, Sparkles, Activity } from 'lucide-react';
import { useGenerationStore, GENRES, MOODS, VOICE_TYPES, DELIVERIES } from '@/pages/veriton/lib/generationStore.js';
...
export default function Create() {
  ...
  const handleGenerate = async () => {
    if (!store.lyrics.trim()) { alert('Please enter some lyrics'); return; }
    setGenerating(true);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-display font-bold crimson-text-gradient mb-2">Create</h1>
        <p className="text-gray-400">Transform your vision into sound</p>
      </div>

      {/* Mode toggle */}
      <div className="glass p-1.5 inline-flex rounded-2xl mb-6 mx-auto block" style={{ borderRadius: 18 }}>
          ...
          {['standard', 'advanced'].map((m) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 flex items-center gap-2 capitalize ${
                store.mode === m ? 'crimson-gradient text-white glow-pulse' : 'text-gray-400 hover:text-white'
              }`}
            >
              {m === 'standard' ? <Sliders className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
              {m} Mode
            </button>
          ))}
        </div>
          ...
          <Section icon={Sparkles} title="Mood">
            <div className="flex flex-wrap gap-2">
              {MOODS.slice(0, 6).map((m) => (
                <GlowChip key={m} label={m} selected={store.mood === m} onClick={() => store.setMood(m)} />
              ))}
            </div>
          </Section>
        </div>
              ...
              <div className="border-t border-white/10 mt-6 pt-6 space-y-6">
                ...
                <Section icon={Upload} title="Voice Capture & Timbre Cloning">
                  <div
                    ...
                    onDrop={(e) => { e.preventDefault(); setDragOver(false); handleVoiceUpload(e.dataTransfer.files[0]); }}
                    onClick={() => fileRef.current?.click()}
                    className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-300 ${
                      dragOver ? 'border-crimson glow-pulse bg-crimson/5' : 'border-white/15 hover:border-white/30'
                    }`}
                  >
                    <input ref={fileRef} type="file" accept=".wav,.mp3,.m4a" className="hidden" onChange={(e) => handleVoiceUpload(e.target.files[0])} />
                    {uploadedVoice ? (
                      <div className="flex items-center justify-center gap-3">
                        <Check className="w-5 h-5 text-crimson" />
                        <span className="text-white text-sm">{uploadedVoice.name}</span>
                      </div>
                    ) : (
                      <>
                        <Upload className="w-8 h-8 text-gray-500 mx-auto mb-3" />
                        <p className="text-gray-400 text-sm">Drag & drop voice sample or click to upload</p>
                        <p className="text-gray-600 text-xs mt-1">.wav, .mp3, .m4a · max 30s / 10MB</p>
                      </>
                    )}
                  </div>
                  <label className="flex items-center gap-3 mt-4 cursor-pointer">
                    <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${consent ? 'crimson-gradient border-crimson' : 'border-white/20'}`}>
                    ...
                    <span className="text-xs text-gray-400">I own the rights to this voice and consent to AI processing</span>
                  </label>
                </Section>
                    ...
                    {['Male', 'Female', 'Both'].map((g) => (
                      <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-sm font-medium transition-all ${
                          store.gender === g ? 'crimson-gradient text-white glow-pulse' : 'glass text-gray-400 hover:text-white'
                        }`}
                      >
                        {g}
                      </button>
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      {store.voiceTypes.map((vt) => (
                        <div key={vt} className="flex items-center gap-4">
                          <span className="text-xs text-gray-300 w-20">{vt}</span>
                          <input
                            type="range"
                            min="0"
                            max="100"
                            value={store.voiceBlend[vt] || 50}
                            onChange={(e) => store.setVoiceBlend(vt, Number(e.target.value))}
                            className="crimson-slider flex-1"
                          />
                          <span className="text-xs text-crimson w-10 text-right">{store.voiceBlend[vt] || 50}%</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <Section icon={Music2} title="Genre Blending">
                  <div className="flex items-center gap-3 mb-4">
                    <button
                      onClick={() => store.setGenreBlendActive(!store.genreBlendActive)}
                      className={`relative w-12 h-6 rounded-full transition-all ${store.genreBlendActive ? 'crimson-gradient' : 'bg-white/10'}`}
                    >
                      <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${store.genreBlendActive ? 'left-7' : 'left-1'}`} />
                    </button>
                    <span className="text-sm text-gray-400">Enable genre blending</span>
                  </div>
                  {store.genreBlendActive && (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-4">
                        <select value={store.genre} onChange={(e) => store.setGenre(e.target.value)} className="px-4 py-3 glass rounded-xl text-white outline-none cursor-pointer">
                          {GENRES.map((g) => <option key={g} value={g} className="bg-[#0A0A0A]">{g}</option>)}
                        </select>
                        <select value={store.genreSecondary} onChange={(e) => store.setGenreSecondary(e.target.value)} className="px-4 py-3 glass rounded-xl text-white outline-none cursor-pointer">
                          {GENRES.map((g) => <option key={g} value={g} className="bg-[#0A0A0A]">{g}</option>)}
                        </select>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="text-xs text-white w-16">{store.genre} {store.genreBlend}%</span>
                        <input
                          type="range"
                          min="0"
                          max="100"
                          value={store.genreBlend}
                          onChange={(e) => store.setGenreBlend(Number(e.target.value))}
                          className="crimson-slider flex-1"
                        />
                        <span className="text-xs text-white w-16 text-right">{100 - store.genreBlend}% {store.genreSecondary}</span>
                      </div>
                    </div>
                  )}
                </Section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Section icon={Volume2} title="Delivery">
                    <select value={store.delivery} onChange={(e) => store.setDelivery(e.target.value)} className="w-full px-4 py-3 glass rounded-xl text-white outline-none cursor-pointer">
                      {DELIVERIES.map((d) => <option key={d} value={d} className="bg-[#0A0A0A]">{d}</option>)}
                    </select>
                  </Section>

                  <Section icon={Gauge} title="BPM">
                    <div className="flex items-center gap-2">
                      <button onClick={() => store.setBpm(store.bpm - 1)} className="w-10 h-10 glass rounded-xl text-white hover:border-crimson/30">−</button>
                      <input
                        type="number"
                        value={store.bpm}
                      ...
                      />
                      <button onClick={() => store.setBpm(store.bpm + 1)} className="w-10 h-10 glass rounded-xl text-white hover:border-crimson/30">+</button>
                      <button onClick={handleDetectBpm} className="px-4 py-3 glass rounded-xl text-crimson text-sm font-medium hover:border-crimson/30 flex items-center gap-1">
                        <Activity className="w-4 h-4" /> Detect
                      </button>
                    </div>
                    {detectedBpm && <p className="text-xs text-crimson mt-2">Detected: {detectedBpm} BPM</p>}
                  </Section>
                </div>
              </div>
            </motion.div>
...
function GeneratingScreen() {
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center">
      ...
      />
      <h2 className="text-2xl font-display font-bold crimson-text-gradient mb-2">Generating...</h2>
