...
export default function AiInsights() {
  const [insights, setInsights] = usePersistentState<string[]>("dashboard_ai_insights", []);
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
        ...
        {insights.length === 0 ? (
          ...
          insights.map((insight, i) => (
            <div key={i} className="glass-crimson rounded-lg p-3">
              <div className="flex items-start gap-2">
                <Brain size={12} className="text-primary/60 mt-0.5 shrink-0" />
                <div className="text-[11px] text-white/70 leading-relaxed">{insight}</div>
              </div>
            </div>
          ))
        )}
      ...
      >
        {loading ? <Loader2 size={11} className="animate-spin" /> : <RefreshCw size={11} />} ANALYZE
      </button>
    </div>
  );
}
