...
type AgentStatusEntry = { name: string; lastRun: { ts: number; status: string } | null; totalRuns: number };
      ...
      <div className="flex-1 space-y-2 overflow-y-auto">
        {loading ? (
          ...
          agents.map((agent) => {
            ...
            return (
              <div key={agent.id} className="glass rounded-lg p-3 border border-white/5 hover:border-primary/20 transition-colors group">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`flex items-center gap-0.5 text-[10px] ${s?.lastRun ? (ok ? "text-emerald-400" : "text-red-400") : "text-white/20"}`}>
                        {s?.lastRun ? <CheckCircle2 size={11} /> : <Circle size={11} />}
                        {s?.lastRun ? (ok ? "Ran OK" : "Last run failed") : "Never run"}
                      </span>
                    </div>
                ...
                <div className="mt-2 pt-2 border-t border-white/5 flex items-center justify-between">
                  <span className="text-[10px] text-white/20">Total runs:</span>
                  <span className="text-[10px] text-white/35">{s?.totalRuns ?? 0}</span>
                </div>
              </div>
            );
          })
