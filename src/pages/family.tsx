...
export default function FamilyPanel() {
  ...
  const updateSelected = (field: keyof Person, value: string) => {
    if (!selected) return;
    setPeople(
      people.map((p) => (p.id === selected.id ? { ...p, [field]: value } : p)),
    );
  };
          ...
          {selected ? (
            <div className="w-full max-w-sm p-6">
              ...
              <div className="space-y-2">
                {(
                  [
                    ...
                    ["notes", "Notes"],
                  ] as [keyof Person, string][]
                ).map(([field, label]) => (
                  <div key={field} className="flex items-center gap-2">
                    <label className="text-[10px] text-white/30 w-24 shrink-0">
                      {label}
                    </label>
                    <input
              ...
              <div className="mt-4 grid grid-cols-3 gap-2">
                ...
                {[
                  ...
                  <div
                    key={a.label}
                    className="flex flex-col items-center gap-1 p-2 glass rounded border border-white/5 text-white/30"
                  >
                    {a.icon}
            ...
            <div className="text-center max-w-sm">
              ...
              <div className="text-[10px] text-white/15 leading-relaxed">
                Fields: Photo, Name, Contact info, All socials, Birthday, Fav
                food, Fav movie, Likes, Dislikes, Milestones, Notes & more
              </div>
            </div>
          )}
        </div>
