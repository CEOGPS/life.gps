...
export default function OmniSearchPanel() {
  ...
  const doSearch = () => {
    ...
    setSearched(true);
    setRecent((prev) => {
      const cleaned = (Array.isArray(prev) ? prev : []).filter(
        (r) => r.toLowerCase() !== q.toLowerCase(),
      );
      return [q, ...cleaned].slice(0, 8);
    });
  };
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              style={{
                ...
                boxShadow: "0 0 20px oklch(0.55 0.22 20 / 8%)",
              }}
              onFocus={(e) =>
                (e.target.style.borderColor = "oklch(0.55 0.22 20 / 60%)")
              }
              onBlur={(e) =>
                (e.target.style.borderColor = "oklch(0.55 0.22 20 / 25%)")
              }
            />
          </div>
          <button
            onClick={doSearch}
            className="px-5 rounded-xl glass-crimson text-primary text-sm font-display hover:glow-crimson transition-all"
          >
            SEARCH
          </button>
          {query && (
        ...
        {!searched ? (
              ...
              {[
                {
                  ...
                  desc: "Find the source of any image",
                },
                {
                  icon: <MapPin size={16} />,
                ...
                },
                {
                  ...
                  title: "Email Finder",
                  desc: "Find emails for any domain",
                },
              ].map((f) => (
                <div
                  key={f.title}
                  className="glass rounded-xl p-4 border border-white/8 hover:border-primary/20 cursor-pointer transition-all"
                >
                  <span className="text-primary/70 mb-2 block">{f.icon}</span>
                  <div className="text-xs text-white/75 font-medium mb-1">
                    {f.title}
                  </div>
                  <div
                    className="text-[9px]"
            ...
            {recent.length > 0 && (
              <div className="glass rounded-xl border border-white/8 p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <button
                    onClick={() => setRecent([])}
                    className="ml-auto text-[9px] text-white/25 hover:text-white/60"
                  >
                    CLEAR
                  </button>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {recent.map((r) => (
                    <button
                      key={r}
                      onClick={() => runQuery(r)}
                      className="flex items-center gap-1 px-2.5 py-1 rounded-full border border-white/8 text-[11px] text-white/40 hover:border-primary/30 hover:text-primary/60 transition-colors"
                    >
                      <Clock size={9} /> {r}
                    </button>
                  ))}
              ...
              <div className="space-y-1.5">
                {TRENDING.map((t) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-white/55 group-hover:text-white/85">
                      {t}
                    </span>
                    <ArrowRight
                      size={10}
                      className="ml-auto text-white/15 group-hover:text-primary/50"
                    />
                  </button>
                ))}
              </div>
          ...
          <div className="glass rounded-xl border border-white/8 p-6 flex flex-col items-center justify-center min-h-[300px]">
            ...
            >
              Wire search APIs (Exa, Google, Bing, DuckDuckGo) to populate
              results
            </div>
          </div>
        )}
