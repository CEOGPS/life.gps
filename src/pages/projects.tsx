          ...
          <div className="grid grid-cols-4 gap-3">
            {STAGES.map((s, i) => {
              ...
              return (
                <div
                  key={s}
                  className="glass rounded-xl border border-white/8 flex flex-col overflow-hidden"
                >
                  <div
                    className={`p-3 border-b border-white/5 flex items-center justify-between ${i === 0 ? "border-l-2 border-l-primary/50" : ""}`}
                  >
                  ...
                  <div className="p-2 flex-1 min-h-[200px] flex flex-col gap-1.5">
                    {stageProjects.map((p) => (
                      <div
                        key={p.id}
                        className="glass rounded-lg p-2 border border-white/6 group"
                      >
                        <div className="flex items-start justify-between gap-1">
                          ...
                          <button
                            onClick={() => remove(p.id)}
                            className="opacity-0 group-hover:opacity-100 text-white/20 hover:text-white/50 shrink-0"
                          >
                            <X size={10} />
                          </button>
                        </div>
            ...
            <div className="flex-1 grid grid-cols-4 gap-4">
              {[
                ...
                <div key={m as string} className="text-center">
                  ...
                  <div className="text-lg text-white/50 font-display mt-0.5">
                    {v}
                  </div>
                </div>
              ))}
