...
function avgMoodShift(sessions) {
  ...
  const avg =
    valid.reduce(
      (acc, s) => acc + (moodScore(s.moodAfter) - moodScore(s.moodBefore)),
      0,
    ) / valid.length;
  return avg;
}
...
export default function GameStateOptimizer({ onBack }) {
  ...
  // GameState tab
  const [gsContext, setGsContext] = useState({
    situation: "",
    energy: "",
    goals: "",
    gaming: "",
  ...
  const runMeetups = useCallback(async () => {
    ...
    const p = `I'm a marketing business owner in Atlanta. Recent games: ${
      sessions
        .slice(0, 3)
        .map((s) => s.game)
        .join(", ") || "various"
    }. My goals: ${advGoals.join(", ") || "scale business, network"}.`;
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: `linear-gradient(135deg, ${C.orange}18, ${C.purple}10)`,
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: C.orange }}>
              GameState Optimizer
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd" }}>
              Gaming as recovery, creativity, and networking fuel
            </div>
          </div>
          {sessions.length > 0 && (
                ...
                <div style={{ textAlign: "center" }}>
                  <div
                    style={{
                      fontSize: 16,
                      fontWeight: 800,
                      color: avgMoodShift(sessions) < 0 ? C.teal : C.red,
                    }}
                  >
                    {avgMoodShift(sessions) < 0 ? "+" : "−"}
                    {Math.abs(avgMoodShift(sessions)).toFixed(1)}
                  </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
        ...
        {activeTab === "gamestate" && (
          <div style={{ maxWidth: 620 }}>
            ...
            >
              Analyze Your Life GameState
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
              Describe your current situation — get a strategic assessment and
              your optimal gaming window
            </div>
              ...
              {[
                {
                  ...
                  ph: "e.g. Scaling my agency, two big proposals pending, behind on content...",
                },
                {
                  key: "energy",
                  label: "⚡ Energy & Schedule",
                  ph: "e.g. Tired by 7pm, mornings sharp, family dinner 6-7pm daily...",
                },
                {
                  key: "goals",
                  label: "🎯 Goals This Month",
                  ph: "e.g. Close 3 clients, launch email campaign, more family time...",
                },
                {
                  key: "gaming",
                  label: "🎮 Your Gaming Habits",
                  ph: "e.g. Play COD 1hr after kids sleep, mostly weekends, love strategy games...",
                },
              ].map((f) => (
                <div key={f.key} style={card}>
                  ...
                  <textarea
                    value={gsContext[f.key]}
                    onChange={(e) =>
                      setGsContext((c) => ({ ...c, [f.key]: e.target.value }))
                    }
                    placeholder={f.ph}
            ...
            <button
              onClick={runGameState}
              disabled={gsLoading || !gsContext.situation.trim()}
              style={{
                ...btnPrimary(C.orange),
                width: "100%",
                padding: 12,
                opacity: gsContext.situation.trim() ? 1 : 0.4,
              }}
            >
              {gsLoading
                ? "⚡ Analyzing GameState..."
                : "🎮 Analyze My GameState"}
            </button>
            ...
            {gsResult && (
              ...
              >
                ...
                {gsResult.lifeModeReason && (
                  <div
                    style={{ fontSize: 12, color: "#8892a4", padding: "0 4px" }}
                  >
                    {gsResult.lifeModeReason}
                  </div>
                )}
                ...
                {gsResult.goldenWindow && (
                  ...
                  >
                    ...
                    >
                      <span
                        style={{ fontSize: 20, fontWeight: 800, color: C.gold }}
                      >
                        {gsResult.goldenWindow.time}
                      </span>
                      <span style={{ fontSize: 11, color: "#6aaedd" }}>
                        {gsResult.goldenWindow.durationMin} min session
                      </span>
                    </div>
                    <div style={{ fontSize: 12, color: "#c8c8d0" }}>
                      {gsResult.goldenWindow.why}
                    </div>
                  </div>
                )}
          ...
          <div>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 11, color: "#6aaedd" }}>
                  Track gaming sessions to unlock pattern insights
                </div>
              </div>
              <button
                onClick={() => setShowLogForm((f) => !f)}
                style={btnSecondary(C.orange)}
              >
              ...
              >
                ...
                >
                  <div>
                    ...
                    <input
                      value={logForm.game}
                      onChange={(e) =>
                        setLogForm((f) => ({ ...f, game: e.target.value }))
                      }
                  ...
                  <div>
                    ...
                    <select
                      value={logForm.genre}
                      onChange={(e) =>
                        setLogForm((f) => ({ ...f, genre: e.target.value }))
                      }
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input
                    ...
                    placeholder="Highlight, funny moment, breakthrough, idea sparked..."
                    style={inp}
                  />
                </div>
                <button
                  onClick={addSession}
                  disabled={!logForm.game.trim()}
                  style={{
                    ...btnPrimary(C.orange),
                    opacity: logForm.game.trim() ? 1 : 0.4,
                  }}
                >
                  Save Session
                </button>
            ...
            {sessions.length === 0 ? (
              ...
              >
                ...
                <div style={{ fontSize: 13 }}>
                  No sessions logged yet. Start tracking to unlock insights.
                </div>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button
                        onClick={() => {
                          setInsightSession(
                            `Game: ${s.game} (${s.genre}, ${s.durationMin}min). ${s.notes || "Good session."} Mood: ${s.moodBefore} → ${s.moodAfter}`,
                          );
                          setActiveTab("insight");
                        }}
                        style={btnSecondary(C.purple)}
        ...
        {activeTab === "advisor" && (
          <div style={{ maxWidth: 620 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
              Games recommended based on your life goals and current energy
            </div>
            ...
            <div style={card}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {["Low 😴", "Medium 😊", "High 🔥"].map((e) => (
                    <button
                      key={e}
                      onClick={() => setAdvEnergy(e)}
                      style={{
                        flex: 1,
                        padding: "7px",
                        borderRadius: 8,
                        border: `1px solid ${advEnergy === e ? C.orange : "rgba(255,255,255,0.08)"}`,
                        background:
                          advEnergy === e ? `${C.orange}15` : "transparent",
                        color: advEnergy === e ? C.orange : "#8892a4",
                        fontSize: 11,
                        cursor: "pointer",
                        fontWeight: advEnergy === e ? 700 : 400,
                      }}
                    >
                      {e}
                    </button>
              ...
              <button
                onClick={runAdvisor}
                disabled={advLoading || advGoals.length === 0}
                style={{
                  ...btnPrimary(C.orange),
                  width: "100%",
                  opacity: advGoals.length > 0 ? 1 : 0.4,
                }}
              >
                {advLoading
                  ? "Finding your games..."
                  : "🕹️ Get Game Recommendations"}
              </button>
            </div>
                ...
                {advResult.picks?.map((p, i) => (
                  <div
                    ...
                    style={{
                      ...card,
                      marginBottom: 10,
                      borderLeft: `3px solid ${[C.orange, C.purple, C.teal][i % 3]}`,
                    }}
                  >
                    <div
                      style={{
        ...
        {activeTab === "insight" && (
          <div style={{ maxWidth: 620 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
              Turn a gaming session into shareable content, team stories, or
              creative fuel
            </div>
            ...
            <div style={{ ...card, marginBottom: 14 }}>
              ...
              >
                DESCRIBE THE SESSION OR MOMENT
              </label>
              <textarea
                value={insightSession}
                onChange={(e) => setInsightSession(e.target.value)}
              ...
              <button
                onClick={runInsight}
                disabled={insightLoading || !insightSession.trim()}
                style={{
                  ...btnPrimary(C.purple),
                  marginTop: 10,
                  width: "100%",
                  opacity: insightSession.trim() ? 1 : 0.4,
                }}
              >
                {insightLoading
                  ? "Forging insights..."
                  : "✂️ Forge Content & Insights"}
              </button>
            </div>
              ...
              >
                ...
                {[
                  ...
                  insightResult[key] ? (
                    <div
                      key={key}
                      style={{ ...card, border: `1px solid ${col}20` }}
                    >
                      <div
                        style={{
                          display: "flex",
                      ...
                      >
                        ...
                        <button
                          onClick={() => copy(insightResult[key], key)}
        ...
        {activeTab === "meetups" && (
          <div style={{ maxWidth: 620 }}>
            ...
            >
              Atlanta Gaming Meetups & Opportunities
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
              Turn your gaming passion into networking and client acquisition in
              Atlanta
            </div>
            ...
            {!meetupResult ? (
              <div style={{ textAlign: "center", padding: 40 }}>
                ...
                >
                  Get 5 AI-generated strategies to convert your gaming hobby
                  into real Atlanta networking and business opportunities.
                </div>
                <button
                  onClick={runMeetups}
              ...
              >
                ...
                {meetupResult.opportunities?.map((o, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      style={{ ...card, borderLeft: `3px solid ${col}` }}
                    >
                      <div
                        style={{
                          display: "flex",
