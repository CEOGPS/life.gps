...
function AgentDetailView({
  ...
  const removeMemory = (idx: number) =>
    onChange({
      ...agent,
      memories: agent.memories.filter((_, i) => i !== idx),
    });
      ...
      <div className="w-72 shrink-0 flex flex-col gap-3 overflow-y-auto pr-1">
        ...
        <div className="glass-crimson rounded-xl p-4 border border-primary/20">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <div
                className="text-white/80 font-display text-sm"
                style={TITLE_STYLE}
              >
                {agent.name}
              </div>
        ...
        <div className="glass rounded-xl p-3 border border-white/8">
          <div
            className="text-[9px] font-display tracking-wider mb-1.5"
        ...
        <div className="glass rounded-xl p-3 border border-white/8">
          ...
          <input
            ...
            onChange={(e) =>
              onChange({ ...agent, personality: e.target.value })
            }
        ...
        <div className="glass rounded-xl p-3 border border-white/8">
          <div
            className="text-[9px] font-display tracking-wider mb-2"
            style={{ color: LABEL_COLOR }}
          >
            SKILLS
          </div>
          ...
          {agent.memories.length === 0 ? (
            ...
            <div className="flex flex-col gap-1 mb-2">
              {agent.memories.map((m, i) => (
                ...
                >
                  <span className="flex-1 text-[10px] text-white/55 leading-relaxed">
                    {m}
                  </span>
                  <button
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div
                        className="text-xs text-white/80 font-display"
                        style={TITLE_STYLE}
                      >
                        {a.name}
                      </div>
                      <div
                        className={`flex items-center gap-1 text-[9px] ${s.color}`}
                      >
                        {s.icon} {s.label}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {a.skills.slice(0, 3).map((sk) => (
                      <span
                        key={sk}
                        className="text-[9px] px-1.5 py-0.5 glass rounded-full text-white/35 border border-white/8"
                      >
                        {sk}
                      </span>
                    ))}
