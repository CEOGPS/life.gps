...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* Top logo area */}
      <div
        className="flex items-center justify-between px-3 py-3.5 border-b shrink-0"
        style={{ borderColor: "oklch(0.55 0.22 20 / 10%)" }}
      >
        {!collapsed && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow: "0 0 10px oklch(0.55 0.22 20 / 30%)",
              }}
            >
              <span
                ...
                style={{
                  ...
                  textShadow: "0 0 8px oklch(0.55 0.22 20 / 80%)",
                }}
              >
                L
              </span>
            </div>
            <span className="font-display text-[10px] tracking-[0.18em] text-white/65 uppercase">
              LifeOS
            </span>
          </motion.div>
        )}
          ...
          <div key={group.label}>
            {!collapsed && (
              <div className="px-3 mb-1 mt-1">
                <span
                  className="text-[8px] tracking-[0.25em] font-display"
              ...
              {group.items.map((item) => {
                ...
                return (
                  <Link key={item.id} to={item.path}>
                    ...
                    >
                      <span
                        className={`shrink-0 ${isActive ? "text-primary" : "text-white/30 group-hover:text-white/60"}`}
                      >
                        {ICON_MAP[item.icon]}
                      </span>
                      {!collapsed && (
                        <span
                          ...
                          style={
                            isActive ? { color: "oklch(0.65 0.22 20)" } : {}
                          }
                        >
                          {item.label}
                        </span>
      ...
      <div
        ...
        style={{ borderColor: "oklch(0.55 0.22 20 / 8%)" }}
      >
        {!collapsed ? (
          <div
            className="text-[8px] font-display tracking-widest text-center"
            style={{ color: "oklch(0.75 0.15 175 / 40%)" }}
          >
            v2.0 LIFEOS
          </div>
        ) : (
