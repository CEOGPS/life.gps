// @ts-nocheck -- large presentational UI kit converted from .jsx; full typing deferred
    ...
    >
      ...
      <div style={innerStyle}>
        ...
        >
          ...
          <div style={{ padding: 14, flex: 1, minHeight: 0, ...bodyStyle }}>
            {children}
          </div>
        </div>
      </div>
...
export function GrowthArrow({ value, prev }) {
  ...
  if (Math.abs(pct) < 0.05)
    return <span style={{ fontSize: 10, color: "var(--t3)" }}>0%</span>;
    ...
    >
      {data.map((d, i) => {
        ...
        return (
          ...
          >
            <div style={{ fontSize: 9, color: fill, fontWeight: 700 }}>
              {fmt(d.v)}
            </div>
            <div
              style={{
...
export function LineChart({ data, color = HEX.crimson, height = 64 }) {
  if (!data || data.length < 2)
    return <Empty icon="📈" text="Not enough history yet" />;
