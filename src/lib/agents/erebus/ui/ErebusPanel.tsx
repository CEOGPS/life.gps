// ErebusPanel.tsx — v15 with Presence + Settings + Initiation Engine
...
import { InitiationEngine } from "./InitiationEngine";
...
// ── Chat Tab ──────────────────────────────────────────────────────────────────
function ChatTab({ core, wakeState, setWakeState, onAvatarVideo, pendingMessage, onDismissPending }) {
  ...
  const hasAddedPendingRef = useRef(false);
  ...
  useEffect(() => {
    if (pendingMessage && !hasAddedPendingRef.current) {
      hasAddedPendingRef.current = true;
      const msg = {
        id: idRef.current++,
        role: "erebus",
      ...
      setTimeout(() => {
        ...
        hasAddedPendingRef.current = false;
      }, 5000);
    }
  ...
  }, [input, files, loading, core, useAgent, setWakeState, onDismissPending]);
...
// ─── Control Tab ─── with Settings + Presence ──────────────────────────────
function ControlTab({ core, wakeState, onSleep, initiationEngine }) {
  ...
  const [settings, setSettings] = useState(() => {
    return storageGet("erebus_settings", DEFAULT_SETTINGS);
  });
  ...
  // ── Settings Handlers ──
  const saveSettings = () => {
    storageSet("erebus_settings", settings);
  ...
  const resetSettingsToDefault = () => {
    if (window.confirm("Reset all settings to defaults?")) {
      ...
      storageSet("erebus_settings", DEFAULT_SETTINGS);
    }
  };

  const handleEmergencyPause = () => {
    const newState = !settings.emergencyPaused;
    setSettings(prev => ({ ...prev, emergencyPaused: newState }));
    if (newState) {
      initiationEngine?.stop();
      core.setPaused(true);
    } else {
    ...
    storageSet("erebus_settings", { ...settings, emergencyPaused: newState });
  };
  ...
  if (section === "presence") {
    const state = initiationEngine?.getState() || {
      ...
      log: [],
      isDND: false,
      personality: { talkativeness: 6, expressiveness: 7, playfulness: 5, directness: 6, empathy: 8 },
      triggers: { time_morning: true, time_evening: true, calendar: true, tasks: true, crm: true, inactivity: true, curiosity: true, context: false },
    ...
    return (
      <ErebusPresence
        ...
        onUpdatePersonality={(trait, value) => initiationEngine?.updatePersonality(trait, value)}
        onUpdateTrigger={(trigger, value) => initiationEngine?.updateTrigger(trigger, value)}
        onUpdateCooldown={(seconds) => initiationEngine?.updateCooldown(seconds)}
        onClearLog={() => initiationEngine?.clearLog()}
        onSetEmotion={(emotion) => {
          if (initiationEngine) {
            initiationEngine.setEmotion(emotion);
          }
        }}
    ...
    return (
      <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
        <div style={{ fontSize: 12, color: C.t2, marginBottom: 16, lineHeight: 1.6 }}>
          Your settings — not restrictions. <span style={{ color: C.eHi }}>You</span> control Erebus.
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["Full", "Ask First", "Read Only", "Off"].map((level) => {
              const levelKey = level.toLowerCase().replace(" ", "_");
              const isActive = settings.permissionLevel === levelKey;
              return (
                <button
                  key={level}
                  onClick={() => setSettings(prev => ({ ...prev, permissionLevel: levelKey }))}
                  style={{
                    padding: "6px 16px",
                    borderRadius: 20,
                    fontSize: 11,
                    ...
                    border: isActive ? `1px solid ${C.e}44` : "1px solid rgba(255,255,255,0.08)",
          ...
          <div style={{ fontSize: 10, color: C.t3, marginTop: 4 }}>
            {settings.permissionLevel === "full" && "✦ Erebus can act autonomously. You trust him."}
            {settings.permissionLevel === "ask_first" && "✦ Erebus asks before any action."}
            {settings.permissionLevel === "read_only" && "✦ Erebus can only read data, not change it."}
            {settings.permissionLevel === "off" && "✦ Erebus is in observation mode only."}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            {[
              ...
              { key: "canAccessFiles", label: "Access files" },
              { key: "canModifySystem", label: "Modify system" },
              { key: "canMakePurchases", label: "Make purchases" },
              { key: "canBrowseWeb", label: "Browse web" },
            ].map((action) => {
              const isActive = settings[action.key] ?? false;
              return (
                <div
                  key={action.key}
                  ...
                  style={{
                    ...
                    border: isActive ? `1px solid ${C.e}44` : "1px solid rgba(255,255,255,0.06)",
                    cursor: "pointer",
                  }}
                >
                  <div
                    style={{
                      ...
                      height: 14,
                      borderRadius: 3,
                      border: `1.5px solid ${isActive ? C.e : C.t3}`,
                  ...
                  <span style={{ fontSize: 11, color: isActive ? C.text : C.t3 }}>
                    {action.label}
                  </span>
                </div>
              );
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            {[
              { key: "allowPolitics", label: "Politics" },
              { key: "allowFinance", label: "Finance advice" },
              { key: "allowMedical", label: "Medical advice" },
              { key: "allowLegal", label: "Legal advice" },
              { key: "allowAdult", label: "Adult content" },
              { key: "allowPersonal", label: "Personal topics" },
            ].map((topic) => {
              const isActive = settings[topic.key] ?? false;
              return (
                <div
                  key={topic.key}
                  onClick={() => setSettings(prev => ({ 
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSettings(prev => ({ 
                  ...prev, 
                  maxUnpromptedPerHour: parseInt(e.target.value) 
                }))}
                style={{
                  width: "100%",
                  accentColor: C.e,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
            <div
              ...
              style={{
                ...
                borderRadius: 9,
                cursor: "pointer",
                background: settings.requireConfirmation ? C.e : "rgba(255,255,255,0.1)",
                transition: "background .2s",
                display: "flex",
                alignItems: "center",
                padding: "0 3px",
                justifyContent: settings.requireConfirmation ? "flex-end" : "flex-start",
              }}
            >
              <div
                style={{
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {settings.emergencyPaused && (
              <span style={{ fontSize: 10, color: C.red }}>
                ⚠ All autonomy is paused. Erebus will not act without your explicit request.
              </span>
            )}
            {!settings.emergencyPaused && (
              <span style={{ fontSize: 10, color: C.t3 }}>
            ...
            <button
              onClick={() => {
                if (settings.overrideCode) {
                  setSettings(prev => ({ ...prev, overrideCode: "" }));
                }
              }}
              style={{
                padding: "7px 14px",
  ...
  const [initiationEngine] = useState(() => {
    ...
    engine.setEvents({
      onInitiation: (message) => {
        ...
        console.log(`[Erebus] 🗣️ Unprompted: ${message.text}`);
      },
      onStateChange: (state) => {
        console.log(`[Erebus] State changed: ${state.presence}, ${state.emotionalState}`);
      },
      onPresenceChange: (presence) => {
        console.log(`[Erebus] Presence: ${presence}`);
      },
    ...
    >
      ...
      <div
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          background: "radial-gradient(circle at 50% 30%, rgba(155,114,207,0.1), transparent 70%)",
        }}
      >
        {avatarVideoUrl ? (
          <video
          ...
          <div
            style={{
              ...
              borderRadius: "50%",
              background: C.eDim,
              border: `2px solid ${pendingMessage ? C.orange : C.e}44`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              fontSize: 28,
