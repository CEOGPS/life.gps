...
const SCENARIOS = [
  {
    icon: "💦",
    title: "Ultimate Water Balloon Fight",
    prompt:
      "Plan the ultimate Atlanta water balloon fight at Piedmont Park. Assign everyone roles, create a battle strategy, and argue about the rules.",
  },
  {
    icon: "🏈",
    title: "Falcons Super Bowl Prediction",
    prompt:
      "The Falcons are going to the Super Bowl. React in character, make wild predictions, and plan the watch party of the century in Atlanta.",
  },
  {
    icon: "🍑",
    title: "Open an Atlanta Food Truck",
    prompt:
      "You're all starting an Atlanta food truck together. Pick the name, the menu, the location, and the vibe. Argue hilariously about everything.",
  },
  {
    icon: "🎤",
    title: "Karaoke Night Gone Wrong",
    prompt:
      "It's karaoke night at a random Atlanta bar. Everyone picks the most wrong song for each other. Things escalate.",
  },
  {
    icon: "🏀",
    title: "3-on-3 Basketball Trash Talk",
    prompt:
      "You're all playing pickup basketball in Atlanta. Trash talk in character, claim impossible stats, and argue about who's the GOAT.",
  },
  {
    icon: "🎲",
    title: "Wild Road Trip Debate",
    prompt:
      "You're planning a road trip from Atlanta to Miami. Everyone wants something different. Plan the most chaotic trip imaginable.",
  },
  {
    icon: "🍕",
    title: "Settle the Debate: Best Pizza",
    prompt:
      "Argue about the best pizza in Atlanta. Everyone has a ridiculous opinion. It somehow becomes a life-or-death philosophical debate.",
  },
  {
    icon: "🌩️",
    title: "Survive an Atlanta Storm",
    prompt:
      "A massive storm knocked out power across Atlanta. Plan your survival strategy from each character's perspective. Chaos ensues.",
  },
  {
    icon: "🏆",
    title: "Fantasy League Draft Day",
    prompt:
      "It's fantasy football draft day. Everyone makes picks that make absolutely no sense for their character. Roast each other mercilessly.",
  },
  {
    icon: "🎪",
    title: "Start a Street Circus",
    prompt:
      "You're starting a pop-up street circus in Little Five Points Atlanta. Assign acts based on your characters' 'abilities'.",
  },
];
...
function buildGroupSystem(friends, scenario) {
  const roster = friends
    .map(
      (f) =>
        `- ${f.fantasyName} (${f.title}): ${f.superpower}. Humor: ${f.humorStyle}. Catchphrase: "${f.catchphrase}"`,
    )
    .join("\n");
...
function Bubble({ msg, crew }) {
  ...
  const archetype = friend
    ? ARCHETYPES.find((a) => a.id === friend.archetype)
    : null;
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: "72%" }}>
        ...
        <div
          style={{
            background: isUser ? `${C.blue}15` : `${col}10`,
            border: `1px solid ${isUser ? C.blue : col}25`,
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          opacity: val.trim() && !loading ? 1 : 0.4,
        }}
      >
        Send
      </button>
...
export default function FantasyFriendSimulator({ onBack }) {
  ...
  const generatePersona = useCallback(async () => {
    ...
    const raw = await invokeLLM({
      systemPrompt: buildCreatorSystem(form),
      prompt,
    });
  ...
  function startSoloChat(friend) {
    ...
    setSoloMsgs([
      {
        role: "system",
        content: `${friend.fantasyName} has entered the chat.`,
        ts: Date.now(),
      },
      {
        role: "friend",
        friendId: friend.id,
        content: friend.openingLine || `Greetings! I am ${friend.fantasyName}!`,
        ts: Date.now(),
      },
    ]);
    setSoloHistory([]);
  ...
  async function sendSolo(text) {
    ...
    const raw = await invokeLLM({
      systemPrompt: buildPersonaSystem(soloFriend, { userName: "Chris" }),
      prompt: text,
      conversationHistory: newHistory.slice(-12),
    }).catch((e) => e.message);
    const reply = {
      role: "friend",
      friendId: soloFriend.id,
      content: raw,
      ts: Date.now(),
    };
  ...
  function startGroup() {
    ...
    const openings = groupParticipants
      .map((f) => `${f.fantasyName}: "${f.openingLine || "Let's gooo!"}"`)
      .join("\n");
    setGroupMsgs([
      {
        role: "system",
        content: `Group chat started with ${groupParticipants.map((f) => f.fantasyName).join(", ")}`,
        ts: Date.now(),
      },
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: `linear-gradient(135deg, ${C.blue}15, ${C.purple}10)`,
          borderBottom: `1px solid ${C.blue}25`,
          padding: "14px 20px",
          flexShrink: 0,
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: C.blue }}>
              Fantasy Friend Simulator
            </div>
            <div style={{ fontSize: 11, color: "#6aaedd" }}>
              Hang out with ridiculous, over-the-top fantasy versions of your
              people
            </div>
          </div>
          {crew.length > 0 && (
        ...
        {TABS.map((t) => (
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              whiteSpace: "nowrap",
              background: activeTab === t.id ? `${C.blue}20` : "transparent",
              color: activeTab === t.id ? C.blue : "#6aaedd",
              borderBottom:
                activeTab === t.id
                  ? `2px solid ${C.blue}`
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 11, color: "#6aaedd" }}>
                Consent-based, wildly over-the-top fantasy versions of your
                people
              </div>
            </div>
            {!creating && (
          ...
          {creating && (
            ...
            >
              ...
              >
                ⚖️ Only add people who have explicitly consented or would
                clearly enjoy this. These are private, silly, cartoon versions —
                never for serious purposes.
              </div>
              ...
              >
                {[
                  { key: "realName", label: "Real Name", ph: "e.g. Marcus" },
                  {
                    ...
                    ph: "e.g. College buddy, business partner",
                  },
                ].map((f) => (
                  <div key={f.key}>
                    <label
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                >
                  Their Personality (give the AI something to work with)
                </label>
                <input
                  value={form.personality}
                  onChange={(e) =>
                    setForm((v) => ({ ...v, personality: e.target.value }))
                  }
                  placeholder="e.g. super competitive, obsessed with soccer, always the loudest in the room, big heart"
                  style={inp}
                />
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                >
                  Humor style or inside jokes (optional)
                </label>
                <input
                  value={form.humorNotes}
                  onChange={(e) =>
                    setForm((v) => ({ ...v, humorNotes: e.target.value }))
                  }
                  placeholder="e.g. loves terrible puns, always talks trash at sports, obsessed with BBQ"
                  style={inp}
                />
              </div>
                  ...
                  {ARCHETYPES.map((a) => {
                    ...
                    return (
                      <button
                        key={a.id}
                        onClick={() =>
                          setForm((v) => ({ ...v, archetype: a.id }))
                        }
                        style={{
                          padding: "5px 12px",
                          borderRadius: 20,
                          border: `1px solid ${sel ? a.color : "rgba(255,255,255,0.1)"}`,
                          background: sel ? `${a.color}20` : "transparent",
                          color: sel ? a.color : "#8892a4",
                          fontSize: 11,
                          cursor: "pointer",
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setForm((v) => ({ ...v, consentAck: e.target.checked }))
                  }
                  style={{ marginTop: 2, accentColor: C.blue }}
                />
                <span
                  style={{ fontSize: 11, color: "#8892a4", lineHeight: 1.5 }}
                >
                  {form.realName || "This person"} has consented or would
                  clearly enjoy being in this silly, private simulation.
                  Sessions are ephemeral and never shared without my action.
                </span>
              </label>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button
                  ...
                  disabled={
                    generating || !form.realName.trim() || !form.consentAck
                  }
                  style={{
                    padding: "9px 20px",
                    borderRadius: 10,
                    ...
                    fontWeight: 800,
                    cursor:
                      form.realName.trim() && form.consentAck && !generating
                        ? "pointer"
                        : "not-allowed",
              ...
              {preview && (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 11, color: C.gold, marginBottom: 10 }}
                  >
                    {preview.title}
                  </div>
                  <div
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={generatePersona} style={btnS(C.blue)}>
                      Regenerate
                    </button>
                  </div>
                </div>
          ...
          {crew.length === 0 && !creating ? (
            <div style={{ textAlign: "center", padding: 60, color: "#4a5568" }}>
              ...
              >
                Add fantasy versions of your friends to start the most
                ridiculous group chat in Atlanta.
              </div>
            </div>
          ) : (
            <div
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button
                        ...
                        style={{ ...btnS(C.red), padding: "7px 10px" }}
                      >
                        ×
                      </button>
                    </div>
          ...
          {!soloFriend ? (
            ...
            >
              ...
              <div style={{ padding: 20, overflowY: "auto", flex: 1 }}>
                ...
                <div
                  style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}
                >
                  One-on-one with your fantasy friend
                </div>
                {crew.length === 0 ? (
                  <div
                    style={{
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{ ...btnS(C.blue), marginTop: 12 }}
                    >
                      Go to Crew
                    </button>
                  </div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 10, color: "#6aaedd" }}>
                            {m.humorStyle} humor ·{" "}
                            {
                              ARCHETYPES.find((a) => a.id === m.archetype)
                                ?.label
                            }
                          </div>
                        </div>
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {soloFriend.fantasyName}
                  </div>
                  <div style={{ fontSize: 10, color: "#6aaedd" }}>
                    {soloFriend.title}
                  </div>
                </div>
                <button onClick={() => setSoloFriend(null)} style={btnS(C.red)}>
                  ← Back
                </button>
              </div>
              {/* Messages */}
                      ...
                      {[0, 1, 2].map((i) => (
                        <div
                          ...
                          style={{
                            ...
                            borderRadius: "50%",
                            background: C.blue,
                            animation: `ffPulse 1.2s ${i * 0.2}s ease-in-out infinite`,
                          }}
                        />
                      ))}
                    </div>
              ...
              <ChatInput
                onSend={sendSolo}
                loading={soloLoading}
                placeholder={`Talk to ${soloFriend.fantasyName}...`}
                color={
                  ARCHETYPES.find((a) => a.id === soloFriend.archetype)
                    ?.color || C.blue
                }
              />
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
              <div style={{ maxWidth: 560 }}>
                ...
                <div
                  style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}
                >
                  Pick 2+ crew members. Watch them interact, or jump in.
                </div>
                ...
                {crew.length < 2 ? (
                  ...
                  >
                    <div>
                      You need at least 2 crew members. Add more in the Crew
                      tab.
                    </div>
                    <button
                      onClick={() => setActiveTab("crew")}
                  ...
                  <>
                    <div style={{ marginBottom: 16 }}>
                      ...
                      >
                        SELECT PARTICIPANTS (min 2)
                      </div>
                      {crew.map((m) => {
                        const sel = groupParticipants.some(
                          (p) => p.id === m.id,
                        );
                        const archetype = ARCHETYPES.find(
                          (a) => a.id === m.archetype,
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              padding: "10px 12px",
                              borderRadius: 12,
                              border: `1px solid ${sel ? archetype?.color || C.blue : "rgba(255,255,255,0.08)"}`,
                              background: sel
                                ? `${archetype?.color || C.blue}12`
                                : "transparent",
                              cursor: "pointer",
                              marginBottom: 8,
                            }}
                    ...
                    <button
                      onClick={startGroup}
                      disabled={groupParticipants.length < 2}
                      style={{
                        width: "100%",
                        padding: 12,
                        borderRadius: 12,
            ...
            >
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px" }}>
                {groupMsgs.map((m, i) => {
                  ...
                  if (m.role === "user")
                    return <Bubble key={i} msg={m} crew={crew} />;
                ...
                {groupLoading && (
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: 10, color: "#4a5568", marginLeft: 6 }}
                    >
              ...
              <ChatInput
                ...
                placeholder="Jump into the chaos..."
                color={C.purple}
              />
            </div>
          )}
      ...
      {activeTab === "scenarios" && (
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 20 }}>
            Pick a scenario, then go to Group Room to run it with your crew
          </div>
          <div
            style={{
              display: "grid",
          ...
          >
            {SCENARIOS.map((s) => {
              ...
              return (
                <div
                  key={s.title}
                  onClick={() => setActiveScenario(sel ? null : s)}
                  style={{
                    background: sel ? `${C.gold}12` : "#11131f",
                    border: `1px solid ${sel ? C.gold : "rgba(255,255,255,0.07)"}`,
                    borderRadius: 12,
                    padding: 16,
                    cursor: "pointer",
