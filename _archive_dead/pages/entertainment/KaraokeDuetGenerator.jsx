...
function loadContacts() {
  try {
    return JSON.parse(localStorage.getItem("lifeos1_echo_contacts") || "[]");
  } catch {
    return [];
  }
}
...
function loadJoyMood() {
  try {
    const log = JSON.parse(localStorage.getItem("lifeos1_joy_log") || "[]");
    return log[0]?.category || null;
  } catch {
    return null;
  }
}
...
const STYLES = [
  ...
  { id: "chaotic", label: "🎉 Chaos Anthem", desc: "Full unhinged energy" },
  {
    ...
    desc: "Playful teasing, all love",
  },
  {
    id: "romantic_comedy",
    label: "🎬 Rom-Com Duet",
    desc: "Like a movie moment",
  },
];
...
function SingAlongSection({
  ...
  const singerLabel =
    section.singer === "Singer 1"
      ? singer1Name
      : section.singer === "Singer 2"
        ? singer2Name
        : "Both";
  const singerColor =
    section.singer === "Singer 1"
      ...
      : section.singer === "Singer 2"
        ...
        : C.teal;
  const isMyPart =
    (section.singer === "Singer 1" && userIsSinger1) ||
    (section.singer === "Singer 2" && !userIsSinger1) ||
    section.singer === "Both";
  ...
  return (
    <div
      style={{
        ...
        padding: "14px 16px",
        borderRadius: 12,
        background: isActive ? `${singerColor}12` : "rgba(255,255,255,0.02)",
        border: `1px solid ${isActive ? singerColor : "rgba(255,255,255,0.06)"}`,
        transition: "all 0.3s",
      }}
    >
      <div
      ...
      >
        ...
        <span style={{ fontSize: 11, color: singerColor, fontWeight: 600 }}>
          {singerLabel}
        </span>
        {isMyPart && isActive && (
          <span
      ...
      {(section.lines || []).map((line, i) => (
        <div
          key={i}
          style={{
            fontSize: isActive && activeLine === i ? 16 : 14,
            fontWeight: isActive && activeLine === i ? 800 : 400,
            color: isActive && activeLine === i ? "#fff" : "#8892a4",
            marginBottom: 6,
            lineHeight: 1.5,
            transition: "all 0.2s",
            paddingLeft: isActive && activeLine === i ? 8 : 0,
            borderLeft:
              isActive && activeLine === i
                ? `3px solid ${singerColor}`
...
export default function KaraokeDuetGenerator({ onBack }) {
  ...
  useEffect(() => {
    if (duet) {
      ...
      setTimeout(() => setRevealActive(false), 4000);
    }
  }, [duet]);
  ...
  const generate = useCallback(async () => {
    const partners = [
      ...selectedPartners.map((c) => c.name),
      ...(manualPartner.trim() ? [manualPartner.trim()] : []),
    ];
    ...
    const prompt = [
      ...
      selectedPartners.length > 0 &&
        `Personality notes: ${selectedPartners.map((c) => `${c.name}: ${c.personality || "fun person"}`).join("; ")}`,
    ...
    const raw = await invokeLLM({ systemPrompt: DUET_SYSTEM, prompt }).catch(
      (e) => e.message,
    );
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: `linear-gradient(135deg, ${C.pink}20, ${C.purple}12)`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd" }}>
              Custom AI-written duets with your people — pure joy, zero
              productivity
            </div>
          </div>
          {duet && (
            <div style={{ display: "flex", gap: 8 }}>
              {!saved ? (
                <button
                  onClick={saveDuet}
                  disabled={saving}
                  style={{ ...btnS(C.gold), opacity: saving ? 0.5 : 1 }}
                >
      ...
      >
        {TABS.map((t) => (
          ...
          >
            {t.label}
            {t.badge && (
              <span style={{ marginLeft: 4, color: C.pink, fontSize: 10 }}>
                {t.badge}
              </span>
            )}
          </button>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 24 }}>
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            ...
            >
                      ...
                      return (
                        <button
                          ...
                          style={{
                            padding: "5px 14px",
                            borderRadius: 20,
                            border: `1px solid ${sel ? C.pink : "rgba(255,255,255,0.1)"}`,
                            background: sel ? `${C.pink}20` : "transparent",
                            color: sel ? C.pink : "#8892a4",
                            fontSize: 11,
                            cursor: "pointer",
              ...
              <div>
                <div
                  style={{ fontSize: 10, color: "#4a5568", marginBottom: 6 }}
                >
                  Or type a name:
                </div>
                <input
                ...
                {MOODS.map((m) => (
                  <button
                    ...
                    style={{
                      ...
                      background:
                        mood === m.id ? `${m.color}20` : "transparent",
                      cursor: "pointer",
                      display: "flex",
                      flexDirection: "column",
                      alignItems: "center",
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 10,
                        color: mood === m.id ? m.color : "#8892a4",
                ...
                {STYLES.map((s) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: "7px 10px",
                      borderRadius: 8,
                      border: `1px solid ${style === s.id ? C.purple : "rgba(255,255,255,0.06)"}`,
                      background:
                        style === s.id ? `${C.purple}15` : "transparent",
                      color: style === s.id ? C.purple : "#8892a4",
                      fontSize: 11,
                      cursor: "pointer",
                      textAlign: "left",
                  ...
                  {SETTINGS.map((s) => (
                    <button
                      ...
                      style={{
                        padding: "5px 8px",
                        borderRadius: 8,
                        border: `1px solid ${setting === s ? C.orange : "rgba(255,255,255,0.06)"}`,
                        background:
                          setting === s ? `${C.orange}15` : "transparent",
                        color: setting === s ? C.orange : "#8892a4",
                        fontSize: 10,
                        cursor: "pointer",
                        textAlign: "left",
                        fontWeight: setting === s ? 700 : 400,
                      }}
                    >
                      {s}
                    </button>
            ...
            <div style={{ ...card, padding: 14, marginBottom: 20 }}>
              ...
              >
                INSIDE JOKES OR SPECIAL NOTES (optional but amazing)
              </div>
              <input
                value={extra}
            ...
            <button
              onClick={generate}
              disabled={generating || partners.length === 0}
              style={{
                width: "100%",
                padding: 16,
                borderRadius: 14,
                ...
                color: partners.length > 0 ? "#fff" : "#4a5568",
                fontSize: 15,
                fontWeight: 900,
                cursor:
                  partners.length > 0 && !generating
                    ? "pointer"
                    : "not-allowed",
                letterSpacing: ".03em",
              }}
            ...
            {partners.length === 0 && (
              ...
              >
                Add at least one singing partner above
              </div>
            )}
          </div>
        </div>
          ...
          {!duet ? (
            ...
            >
              ...
              <div style={{ fontSize: 13 }}>
                Generate a duet first in the Setup tab.
              </div>
              <button
              ...
              >
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#6aaedd" }}>
                      {duet.originalSongVibe} · {duet.totalRuntime}
                    </div>
                  </div>
                  {/* Singer toggle */}
                  <div style={{ display: "flex", gap: 6 }}>
                    <button
                      onClick={() => setUserIsSinger1(true)}
                      style={{
                        padding: "5px 12px",
                        borderRadius: 8,
                        border: `1px solid ${userIsSinger1 ? C.pink : "rgba(255,255,255,0.1)"}`,
                        background: userIsSinger1
                          ? `${C.pink}20`
                          : "transparent",
                        color: userIsSinger1 ? C.pink : "#8892a4",
                        fontSize: 10,
                        cursor: "pointer",
                        fontWeight: 700,
                      }}
                    ...
                    <button
                      onClick={() => setUserIsSinger1(false)}
                      style={{
                        padding: "5px 12px",
                        borderRadius: 8,
                        border: `1px solid ${!userIsSinger1 ? C.purple : "rgba(255,255,255,0.1)"}`,
                        background: !userIsSinger1
                          ? `${C.purple}20`
                          : "transparent",
                        color: !userIsSinger1 ? C.purple : "#8892a4",
                        fontSize: 10,
                        cursor: "pointer",
                        fontWeight: 700,
                      }}
                  ...
                  {!singAlong ? (
                    ...
                    <button onClick={stopSingAlong} style={btnS(C.red)}>
                      ■ Stop
                    </button>
                  )}
                </div>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
                  ...
                  <span style={{ fontSize: 10, color: C.purple }}>
                    ■ Singer 2 ({singer2Name})
                  </span>
                ...
                <div style={{ maxWidth: 680 }}>
                  ...
                  {duet.performancePrompts && (
                    ...
                    >
                      ...
                      {Object.entries(duet.performancePrompts).map(
                        ([key, val]) => {
                          const labels = {
                            singer1Stage: "Singer 1 Character",
                            singer2Stage: "Singer 2 Character",
                            chorusDance: "Chorus Move 💃",
                            bridgeGimmick: "Bridge Bit 🎪",
                            finaleInstruction: "Grand Finale 🎆",
                          };
                          return (
                            <div key={key} style={{ marginBottom: 10 }}>
                              ...
                              <span style={{ fontSize: 12, color: "#c8c8d0" }}>
                                {val}
                              </span>
                            </div>
                          );
                  ...
                  {duet.backingTrack && (
                    ...
                    >
                      ...
                      >
                        Search YouTube Karaoke for this or hum the melody
                        described.
                      </div>
                    </div>
                  )}
                  ...
                  {duet.insideJokeLines?.length > 0 && (
                    <div style={{ ...card, padding: 14, marginBottom: 16 }}>
                      ...
                      >
                        💥 THE LINES THAT'LL MAKE YOU LOSE IT
                      </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 11, color: "#6aaedd" }}>
                Auto-expire after 48 hours unless saved
              </div>
            </div>
            <button onClick={loadRecent} style={btnS(C.pink)}>
              Refresh
            </button>
          </div>
          ...
          {loadingRecent ? (
            ...
            <div style={{ textAlign: "center", padding: 60, color: "#4a5568" }}>
              ...
              <div style={{ fontSize: 13 }}>
                No saved duets yet. Generate one and hit Save!
              </div>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {recentDuets.map((d) => {
                ...
                const hoursLeft = Math.max(
                  0,
                  Math.floor((expires - Date.now()) / 3600000),
                );
                const styleColors = {
                  silly: C.orange,
                  heartfelt: C.pink,
                    ...
                    >
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        <div
                          style={{
                            fontSize: 10,
                            color: hoursLeft < 6 ? C.red : "#4a5568",
                          }}
                        >
                          {hoursLeft > 0
