...
function RadarChart({ current, alternate, size = 160 }) {
  ...
  const cVals = [
    current.monthlyRevenue / 200,
    current.happinessScore,
    current.freedomScore,
    current.familyScore,
    100 - current.stressScore,
  ];
  const aVals = [
    alternate.monthlyRevenue / 200,
    alternate.happinessScore,
    alternate.freedomScore,
    alternate.familyScore,
    100 - alternate.stressScore,
  ];
  ...
  function point(val, i, scale = 1) {
    ...
    return {
      x: cx + Math.cos(angle) * r * pct * scale,
      y: cy + Math.sin(angle) * r * pct * scale,
    };
  }
  ...
  return (
    ...
    >
      ...
      <polygon
        points={polygon(cVals)}
        fill={`${C.blue}25`}
        stroke={C.blue}
        strokeWidth={2}
      />
      <polygon
        points={polygon(aVals)}
        fill={`${C.teal}20`}
        stroke={C.teal}
        strokeWidth={2}
        strokeDasharray="4 2"
...
function ScoreRow({ label, current, alternate, invert = false }) {
  ...
  return (
    ...
    >
      <span
        style={{ fontSize: 11, color: "#8892a4", width: 90, flexShrink: 0 }}
      >
        {label}
      </span>
      <div
      ...
      >
        ...
        <div
          style={{
            position: "absolute",
            height: "100%",
            width: `${Math.min(alternate, 100)}%`,
            background: `${C.teal}50`,
            borderRadius: 3,
            border: `1px dashed ${C.teal}80`,
          }}
        />
      </div>
      <span
...
function RiskCard({ risk }) {
  ...
  const sevColor =
    { high: C.red, medium: C.orange, low: C.gold }[risk.severity] || C.orange;
  const sevIcon =
    { high: "🔴", medium: "🟡", low: "🟢" }[risk.severity] || "🟡";
  return (
    <div
      onClick={() => setOpen((o) => !o)}
      style={{
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <span>{sevIcon}</span>
        <span
          style={{ flex: 1, fontSize: 12, fontWeight: 600, color: sevColor }}
        >
          {risk.domain}: {risk.flag}
        </span>
      ...
      {open && (
        <div
          style={{
            marginTop: 8,
            paddingTop: 8,
            borderTop: `1px solid ${sevColor}20`,
          }}
        >
...
const PRESETS = [
  {
    icon: "🎮",
    label: "Full-time streaming + events",
    text: "What if I quit my day job and went full-time into game streaming and local gaming events in Atlanta?",
  },
  {
    icon: "🏠",
    label: "Remote work + relocate",
    text: "What if I switched to fully remote work and moved to a lower cost-of-living area outside Atlanta?",
  },
  {
    icon: "🍳",
    label: "Start a food business",
    text: "What if I launched a weekend pop-up food business using my cooking skills while keeping my current job?",
  },
  {
    icon: "📚",
    label: "Go back to school",
    text: "What if I went back to school part-time for an MBA while running my current business?",
  },
  {
    icon: "🤝",
    label: "Sell business, consult instead",
    text: "What if I sold my current business and pivoted to consulting in my industry?",
  },
  {
    icon: "✈️",
    label: "Semi-retire at 45",
    text: "What if I aggressively cut expenses and built passive income to semi-retire in 5 years?",
  },
];
...
export default function AlternateLifeExplorer({ onBack }) {
  ...
  const [savedSims, setSavedSims] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("lifeos1_alt_sims") || "[]");
    } catch {
      return [];
    }
  });
    ...
    return (
      <div style={{ height: "100%", overflowY: "auto", padding: 28 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          >
            Ask any "what if" life question. Get a multi-month simulation
            grounded in your real finances, family, energy, and Atlanta market
            signals — plus a 30-day test drive plan.
          </div>
        </div>
        ...
        <div style={{ maxWidth: 620, margin: "0 auto" }}>
              ...
              {PRESETS.map((p) => (
                <button
                  ...
                  style={{
                    ...
                    padding: "8px 12px",
                    borderRadius: 10,
                    border: `1px solid ${whatIf === p.text ? C.blue : "rgba(255,255,255,0.08)"}`,
                    background:
                      whatIf === p.text ? `${C.blue}15` : "transparent",
                    color: whatIf === p.text ? C.blue : "#8892a4",
                    cursor: "pointer",
                    textAlign: "left",
                    fontSize: 11,
                  }}
          ...
          <div style={{ ...card, marginBottom: 16 }}>
            ...
            >
              CONTEXT — makes simulation more accurate
            </div>
            <div
              style={{
                display: "grid",
              ...
              {[
                {
                  key: "finances",
                  label: "💰 Finances",
                  ph: "e.g. $6k/mo income, $3k expenses, $20k savings",
                },
                {
                  key: "family",
                  label: "👨‍👩‍👧 Family",
                  ph: "e.g. Married, 2 kids, partner works part-time",
                },
                {
                  key: "energy",
                  label: "⚡ Energy/Schedule",
                  ph: "e.g. Mornings sharp, low by 6pm, weekends free",
                },
                {
                  key: "skills",
                  label: "🎯 Skills",
                  ph: "e.g. 10yrs marketing, decent content creator",
                },
              ].map((f) => (
                <div key={f.key}>
                  <label
                  ...
                  <input
                    value={context[f.key]}
                    onChange={(e) =>
                      setContext((c) => ({ ...c, [f.key]: e.target.value }))
                    }
                    placeholder={f.ph}
                    style={inp}
                  />
          ...
          <button
            ...
            style={{
              ...
              fontSize: 14,
              fontWeight: 800,
              cursor: whatIf.trim() ? "pointer" : "not-allowed",
              opacity: whatIf.trim() ? 1 : 0.4,
            }}
          >
            🌍 Run Life Simulation
          </button>
  ...
  if (phase === "loading")
    return (
      ...
      >
        <div style={{ fontSize: 56, animation: "altSpin 4s linear infinite" }}>
          🌍
        </div>
        ...
        >
          Running multi-domain simulation across finances, family, energy, and
          Atlanta market data...
        </div>
        <div
          style={{
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                fontSize: 10,
                color: C.blue,
                animation: `altPulse 1.8s ease-in-out ${i * 0.25}s infinite`,
              }}
            >
              {s}
            </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          <div>
                ...
                ({ data, col, label }) =>
                  data && (
                    ...
                    >
                      ...
                      <div
                        style={{ fontSize: 18, fontWeight: 800, color: col }}
                      >
            ...
            >
              {sim.currentLife && sim.alternateLife && (
                ...
                >
                  <div
                    style={{ fontSize: 10, color: "#4a5568", marginBottom: 8 }}
                  >
                    <span style={{ color: C.blue }}>─</span> Current &nbsp;{" "}
                    <span style={{ color: C.teal }}>╌</span> Alternate
                  </div>
                  <RadarChart
                    current={sim.currentLife}
                    alternate={sim.alternateLife}
                  />
                </div>
              )}
              {sim.currentLife && sim.alternateLife && (
                <div style={{ flex: 1, minWidth: 220 }}>
                  ...
                  <ScoreRow
                    label="Happiness"
                    current={sim.currentLife.happinessScore}
                    alternate={sim.alternateLife.happinessScore}
                  />
                  <ScoreRow
                    label="Freedom"
                    current={sim.currentLife.freedomScore}
                    alternate={sim.alternateLife.freedomScore}
                  />
                  <ScoreRow
                    label="Family Time"
                    current={sim.currentLife.familyScore}
                    alternate={sim.alternateLife.familyScore}
                  />
                  <ScoreRow
                    label="Stress"
                    current={sim.currentLife.stressScore}
                    alternate={sim.alternateLife.stressScore}
                    invert
                  />
        ...
        {activeTab === "timeline" && (
          <div style={{ maxWidth: 680 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
              How both paths unfold month by month
            </div>
        ...
        {activeTab === "risks" && (
          <div style={{ maxWidth: 560 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
              Tap each flag to reveal the mitigation strategy
            </div>
        ...
        {activeTab === "testdrive" && sim.testDrive && (
          <div style={{ maxWidth: 600 }}>
            ...
            <div style={{ fontSize: 11, color: "#6aaedd", marginBottom: 16 }}>
              A micro-experiment to validate the alternate path before
              committing
            </div>
            ...
            >
              {sim.testDrive.goSignal && (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 12, color: "#c8c8d0", lineHeight: 1.5 }}
                  >
                    {sim.testDrive.goSignal}
                  </div>
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {sim.atlantaAngle && (
                ...
                >
                  ...
                  >
                    🏙️ ATLANTA MARKET ANGLE
                  </div>
                  <div
                    style={{ fontSize: 13, color: "#c8c8d0", lineHeight: 1.7 }}
                  >
                    {sim.atlantaAngle}
                  </div>
                </div>
