...
import { loadCredentials, saveCredential, deleteCredential, migrateFromLocalStorage } from "@/lib/integrationsSupabase.ts";
...
export default function IntegrationsPanel() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (!firebaseUser) return;
      loadCredentials(firebaseUser.email || "").then((creds) => {
        if (Object.keys(creds).length) {
          setAccounts(creds);
          // Migrate any localStorage data if Supabase was empty
          const local = loadAccountsLegacy();
          const hasLocal = Object.keys(local).some(
            (k) => Object.keys(local[k] || {}).filter((e) => e !== "_extras").length > 0
          );
          if (hasLocal) {
            migrateFromLocalStorage(firebaseUser.email || "", local);
          }
        } else {
    ...
    useEffect(() => {
      if (!firebaseUser) return;
      saveAccountsToSupabase(firebaseUser.email || "", accounts);
    }, [accounts, firebaseUser]);

    // Legacy localStorage helpers for migration
    const LEGACY_AK = "lifeos1_accounts_v3";
    function loadAccountsLegacy() {
      try {
        return JSON.parse(localStorage.getItem(LEGACY_AK) || "{}");
      } catch {
        return {};
      }
    }

    async function saveAccountsToSupabase(userEmail, accounts) {
        try {
          const credentials = [];
          for (const [integrationName, accountsMap] of Object.entries(accounts)) {
            for (const [email, slot] of Object.entries(accountsMap)) {
              if (email === "_extras") continue;
              if (slot.status === "on" || slot.apiKey || slot.username || slot.password) {
                credentials.push({
