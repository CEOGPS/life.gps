...
async function checkRateLimit(env, key, windowSec = 60, maxReqs = 30) {
  ...
  val.count++;
  await env.LIFEOS_KV.put(k, JSON.stringify(val), {
...
async function getUserId(env, emailOrId) {
  ...
  if (uuidRegex.test(emailOrId)) {
    const { data } = await supabase(
      env,
      `/rest/v1/users?select=id&id=eq.${emailOrId}`,
    );
    if (data?.length) return data[0].id;
  }
  const { data } = await supabase(
    env,
    `/rest/v1/users?select=id&email=eq.${encodeURIComponent(emailOrId)}`,
  );
  if (data?.length) return data[0].id;
  const { data: newUser } = await supabase(env, "/rest/v1/users", {
    method: "POST",
...
async function saveOAuthTokensToSupabase(
  env,
  provider,
  tokens,
  userId,
  ...
  try {
    ...
    if (!r.ok || t.error) {
      return {
        ...
        detail: t.error_description || t.error || `HTTP ${r.status}`,
        provider,
      };
    }
    const updated = {
    ...
    await env.LIFEOS_KV.put(
      `oauth_${provider}_${accountLabel}`,
...
async function setPrimaryAccount(env, userId, provider, accountEmail) {
  await supabase(
    env,
    `/rest/v1/platform_tokens?user_id=eq.${userId}&platform=eq.${provider}`,
    {
      method: "PATCH",
  ...
  await supabase(
    env,
    `/rest/v1/platform_tokens?user_id=eq.${userId}&platform=eq.${provider}&account_email=eq.${encodeURIComponent(accountEmail)}`,
    {
      method: "PATCH",
      body: JSON.stringify({ is_primary: true }),
    },
...
async function getValidAccessTokenFromDB(
  env,
  provider,
  userId,
  accountEmail = null,
...
function getProviderConfig(env, provider, scope) {
  ...
  const configs = {
    ...
    facebook: {
      ...
      scope:
        "pages_manage_posts pages_read_engagement pages_show_list pages_manage_metadata instagram_basic instagram_content_publish instagram_manage_insights ads_management business_management",
    ...
    instagram: {
      ...
      client_secret: env.META_APP_SECRET || "",
      scope:
        "instagram_basic instagram_content_publish instagram_manage_insights instagram_manage_comments pages_show_list",
      redirect_uri: ownedRedirect,
    },
    twitter: {
...
async function liToken(env) {
  const data =
    (await env.LIFEOS_KV.get(`oauth_linkedin_default`, "json")) ||
    (await env.LIFEOS_KV.get(`oauth_linkedin`, "json"));
...
async function handleOAuthCallback(req, env, url) {
  ...
  try {
    if (stored.provider === "google") {
      ...
      stored.provider === "facebook" ||
      stored.provider === "instagram"
    ) {
      const r = await fetch(
...
export default {
  async fetch(req, env) {
    ...
    if (path.startsWith("/api/settings/")) {
      ...
      if (req.method === "GET")
        return json((await env.LIFEOS_KV.get("settings_" + key, "json")) || {});
      if (req.method === "POST") {
        await env.LIFEOS_KV.put(
          "settings_" + key,
    ...
    if (path.startsWith("/api/kv/")) {
      ...
      if (req.method === "GET")
        return json((await env.LIFEOS_KV.get(key, "json")) || null);
      if (req.method === "POST") {
    ...
    if (path === "/api/upload" && req.method === "POST") {
      ...
      const fileKey =
        formData.get("key") || `${type}/${Date.now()}_${file.name}`;
    ...
    if (path === "/api/oauth/verify" && req.method === "GET") {
      ...
      if (!provider || !userId)
        return json({ ok: false, error: "provider and user_id required" });
      try {
    ...
    if (path === "/api/oauth/accounts-legacy" && req.method === "GET") {
      ...
      const idx =
        (await env.LIFEOS_KV.get(`oauth_index_${provider}`, "json")) || [];
    ...
    if (path === "/api/oauth/set-primary" && req.method === "POST") {
      ...
      if (!user_id || !provider || !account_email)
        return err("user_id, provider, and account_email required");
    ...
    if (path === "/api/telegram/updates" && req.method === "GET") {
      ...
      const res = await fetch(
        `${TELEGRAM_API}/getUpdates?timeout=0&limit=20${offset ? "&offset=" + offset : ""}`,
      );
      return json(await res.json());
    }
    ...
    if (path === "/api/telegram/webhook" && req.method === "POST") {
      ...
      const res = await fetch(
        `${TELEGRAM_API}/setWebhook?url=${encodeURIComponent(wUrl)}`,
      );
      return json(await res.json());
    }
    if (path === "/api/telegram/incoming" && req.method === "POST") {
      ...
      if (msg) {
        ...
        const chatList =
          (await env.LIFEOS_KV.get("telegram_chats", "json")) || [];
        if (!chatList.find((c) => c.id === chatId)) {
          chatList.unshift({
            id: chatId,
            name: msg.chat.title || msg.from?.first_name || "Unknown",
            type: msg.chat.type,
          });
          await env.LIFEOS_KV.put(
            "telegram_chats",
    ...
    if (path === "/api/telegram/messages" && req.method === "GET") {
      ...
      return json(
        (await env.LIFEOS_KV.get(`telegram_chat_${chatId}`, "json")) || {
          messages: [],
          chat: {},
        },
      );
    ...
    // ========================================================================
    const manualMeta = await env.LIFEOS_KV.get(
      "manual_meta_token",
      "json",
    ).catch(() => null);
    const META_TOKEN =
      manualMeta?.access_token || env.META_PAGE_ACCESS_TOKEN || "";
    const META_PAGE_ID =
      (manualMeta?.page_id && manualMeta.page_id.length > 5
        ? manualMeta.page_id
        : null) ||
      env.META_PAGE_ID ||
      "";
    const META_IG_ID =
      (manualMeta?.ig_user_id && manualMeta.ig_user_id.length > 5
        ? manualMeta.ig_user_id
        : null) ||
      env.META_IG_USER_ID ||
      "";
    ...
    if (path === "/api/meta/status" && req.method === "GET") {
      try {
        if (!META_TOKEN || META_TOKEN.trim().length < 10)
          return json({ connected: false, error: "token_missing" });
        let resolvedPageId =
          META_PAGE_ID && META_PAGE_ID.trim().length > 5 ? META_PAGE_ID : null;
        let resolvedIgId =
          META_IG_ID && META_IG_ID.trim().length > 5 ? META_IG_ID : null;
        if (!resolvedPageId) {
          ...
          if (me?.id && !me?.error) {
            ...
            if (!resolvedIgId && me.instagram_business_account?.id)
              resolvedIgId = me.instagram_business_account.id;
          }
          if (!resolvedPageId) {
            ...
            if (accounts?.data?.length) {
              ...
              if (!resolvedIgId && first.instagram_business_account?.id)
                resolvedIgId = first.instagram_business_account.id;
            }
          }
          if (resolvedPageId) {
            const existing =
              (await env.LIFEOS_KV.get("manual_meta_token", "json").catch(
                () => null,
              )) || {};
            await env.LIFEOS_KV.put(
              "manual_meta_token",
        ...
        const page = resolvedPageId
          ? await graphGet(
              `/${resolvedPageId}?fields=name,fan_count,followers_count`,
              META_TOKEN,
            )
          : null;
        const ig = resolvedIgId
          ? await graphGet(
              `/${resolvedIgId}?fields=username,followers_count,media_count`,
              META_TOKEN,
            )
          : null;
        const tokenExpired =
          page?.error?.type === "OAuthException" ||
          ig?.error?.type === "OAuthException";
        if (tokenExpired)
          return json({
            connected: false,
            error: "token_expired",
        ...
        if (!resolvedPageId && !resolvedIgId) {
          ...
          return json({
            ...
            me_response: meDebug,
            token_length: META_TOKEN.length,
          });
        }
        return json({
    ...
    if (path === "/api/meta/pages" && req.method === "GET") {
      return json(
        await graphGet(
          "/me/accounts?fields=name,id,fan_count,access_token",
          META_TOKEN,
        ),
      );
    }
    if (path === "/api/meta/feed" && req.method === "GET") {
      ...
      const cached = await env.LIFEOS_KV.get("manual_meta_token", "json").catch(
        () => null,
      );
      const feedPageId =
        (cached?.page_id && cached.page_id.length > 5
          ? cached.page_id
          : null) || META_PAGE_ID;
      if (!feedPageId || feedPageId.trim().length < 5)
        return json({ data: [], error: "page_id_not_set" });
      return json(
        await graphGet(
          `/${feedPageId}/feed?fields=message,created_time,likes.summary(true),comments.summary(true)&limit=${limit}`,
          META_TOKEN,
        ),
      );
    }
    if (path === "/api/meta/instagram/feed" && req.method === "GET") {
      ...
      return json(
        await graphGet(
          `/${META_IG_ID}/media?fields=id,caption,media_type,thumbnail_url,permalink,like_count,comments_count,timestamp&limit=${limit}`,
          META_TOKEN,
        ),
      );
    }
    if (path === "/api/meta/instagram/insights" && req.method === "GET") {
      return json(
        await graphGet(
          `/${META_IG_ID}/insights?metric=impressions,reach,profile_views&period=day`,
          META_TOKEN,
        ),
      );
    }
    if (path === "/api/meta/ads" && req.method === "GET") {
      return json(
        await graphGet(
          `/${META_AD_ACCT}/campaigns?fields=name,status,objective,budget_remaining,spend_cap&limit=10`,
          META_TOKEN,
        ),
      );
    }
    ...
    if (path === "/api/linkedin/status" && req.method === "GET") {
      ...
      return json({
        connected: !!me?.sub,
        profile: me
          ? { id: me.sub, name: me.name, email: me.email, picture: me.picture }
          : null,
      });
    }
    ...
    if (path === "/api/oauth/token/save" && req.method === "POST") {
      try {
        ...
        if (provider === "facebook" || provider === "instagram") {
          ...
          return json({
            ok: true,
            token_length: finalToken.length,
            name: test.name,
            id: test.id,
          });
        }
      ...
      if (path === "/oauth/authorize" && req.method === "GET") {
        ...
        if (!REGISTERED_CLIENTS[clientId]) {
          return new Response("Unknown or untrusted client_id", {
            status: 400,
          });
        }
        ...
        await env.LIFEOS_KV.put(
          ...
          JSON.stringify({
            ...
            code_challenge_method: url.searchParams.get(
      ...
      */
      ...
      if (path === "/api/oauth/consent" && req.method === "POST") {
        try {
          ...
          let authReq = null;
          if (authz_id) {
            authReq = await env.LIFEOS_KV.get(
              `oauth_authz:${authz_id}`,
              "json",
            );
          }
      ...
      if (path === "/oauth/token" && req.method === "POST") {
        ...
        try {
          ...
          return json(
            { error: "server_error", error_description: e.message },
            500,
          );
        }
      }
    ...
    if (path === "/api/x/user" && req.method === "GET") {
      ...
      const manualX = await env.LIFEOS_KV.get("manual_x_token", "json").catch(
        () => null,
      );
    ...
    if (path === "/api/x/timeline" && req.method === "GET") {
      ...
      const manualX2 = await env.LIFEOS_KV.get("manual_x_token", "json").catch(
        () => null,
      );
      const bearer = manualX2?.bearer_token || env.X_BEARER_TOKEN || "";
      if (!bearer)
        return json({ tweets: [], error: "X_BEARER_TOKEN not configured" });
      try {
    ...
    if (path === "/api/llm/invoke" && req.method === "POST") {
      ...
      try {
        ...
        let chain =
          m === "auto" || !m ? auto : [m, ...auto.filter((x) => x !== m)];
    ...
    if (path === "/api/llm/preference" && req.method === "GET") {
      const preferred =
        (await env.LIFEOS_KV.get("llm_preferred_model")) || "auto";
    ...
    if (path === "/api/email/accounts" && req.method === "GET") {
      const accounts = [];
      const gIdx =
        (await env.LIFEOS_KV.get("oauth_index_google", "json")) || [];
      for (const a of gIdx) {
        accounts.push({
          provider: "gmail",
      ...
      const mIdx =
        (await env.LIFEOS_KV.get("oauth_index_microsoft", "json")) || [];
      for (const a of mIdx) {
        accounts.push({
          provider: "outlook",
          account_label: a.label,
    ...
    if (path === "/api/email/labels" && req.method === "GET") {
      ...
      if (provider === "gmail") {
        ...
        if (!r.ok)
          return json({ error: "gmail_labels_failed", status: r.status }, 502);
        const d = await r.json();
    ...
    if (path === "/api/email/threads" && req.method === "GET") {
      ...
      if (provider === "gmail") {
        ...
        if (!r.ok)
          return json({ error: "gmail_threads_failed", status: r.status }, 502);
        const d = await r.json();
        ...
        const detailed = await Promise.all(
          ids.map(async (id) => {
            try {
              ...
              msgs.forEach((m) =>
                (m.labelIds || []).forEach((l) => labelIds.add(l)),
              );
              return {
                id: th.id,
      ...
      try {
        ...
        if (provider === "gmail") {
          ...
          const mime = [
            ...
            .filter(Boolean)
            .join("\r\n");
          const enc = btoa(unescape(encodeURIComponent(mime)))
            .replace(/\+/g, "-")
            .replace(/\//g, "_")
            .replace(/=+$/, "");
          const r = await fetch(
          ...
          if (!r.ok)
            return json({ error: "gmail_send_failed", status: r.status }, 502);
          const d = await r.json();
          return json({
            ok: true,
            provider: "gmail",
        ...
        if (provider === "brevo") {
          ...
          const r = await fetch("https://api.brevo.com/v3/smtp/email", {
            ...
            body: JSON.stringify({
              ...
              subject,
              htmlContent: body_html || `<pre>${body_text}</pre>`,
              textContent: body_text || undefined,
            }),
          });
          if (!r.ok)
            return json({ error: "brevo_send_failed", status: r.status }, 502);
    ...
    if (path.startsWith("/api/email/thread/") && req.method === "GET") {
      ...
      if (provider === "gmail") {
        ...
        try {
          ...
          if (!r.ok)
            return json(
              { error: "gmail_thread_failed", status: r.status },
              502,
            );
    ...
    const manualYt = await env.LIFEOS_KV.get("manual_yt_config", "json").catch(
      () => null,
    );
    const YT_CHANNEL_ID =
      (manualYt?.channel_id && manualYt.channel_id.trim().length > 5
        ? manualYt.channel_id
        : null) ||
    ...
    if (path === "/api/youtube/channel" && req.method === "GET") {
      try {
        ...
        const YT_HANDLE =
          manualYt?.channel_handle || env.YOUTUBE_CHANNEL_HANDLE || "@ceogps";
        const query =
          YT_CHANNEL_ID && YT_CHANNEL_ID.trim().length > 5
            ? `id=${YT_CHANNEL_ID}`
            : `forHandle=${encodeURIComponent(YT_HANDLE)}`;
        const r = await fetch(
          `https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&${query}&key=${YT_KEY}`,
        );
        const d = await r.json();
        const ch = d.items?.[0];
        if (!ch)
          return json(
            {
              error:
                "Channel not found — set YOUTUBE_CHANNEL_ID or YOUTUBE_CHANNEL_HANDLE in Worker env",
            },
            404,
          );
        return json({
      ...
      try {
        ...
        if (!channelId) {
          ...
          const cr = await fetch(
            `https://www.googleapis.com/youtube/v3/channels?part=id&forHandle=${encodeURIComponent(YT_HANDLE)}&key=${YT_KEY}`,
          );
        ...
        const r = await fetch(
          `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelId}&maxResults=${max}&order=date&type=video&key=${YT_KEY}`,
        );
        const d = await r.json();
      ...
      try {
        ...
        if (extract === "links") {
          ...
          const links = linkMatches
            .map((m) => ({
              href: m[1].startsWith("http")
                ? m[1]
                : new URL(m[1], fetchUrl).href,
              text: m[2]
                .replace(/<[^>]+>/g, "")
                .trim()
                .slice(0, 80),
    ...
    if (path === "/api/browse/search" && req.method === "POST") {
      try {
        ...
        for (const block of blocks.slice(1, limit + 1)) {
          ...
          const snippet = snippM
            ? snippM[1].replace(/<[^>]+>/g, "").trim()
            : "";
          if (title)
            results.push({
              title,
      ...
      try {
        ...
        const upstream = await fetch(
          `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`,
          {
            ...
            body: JSON.stringify({
              text: body.text,
              model_id: body.model || "eleven_monolingual_v1",
    ...
    if (path === "/api/nylas/threads" && req.method === "GET") {
      ...
      const threads = (d.data || []).map((t) => ({
        ...
        snippet: t.snippet || "",
        from: t.from?.[0]?.email || "",
        date: t.latest_message_received_date
          ? t.latest_message_received_date * 1000
          : Date.now(),
        unread: !t.read,
    ...
    if (path.startsWith("/api/nylas/thread/") && req.method === "GET") {
      ...
      const r = await fetch(
        `https://api.us.nylas.com/v3/grants/${grantId}/messages?thread_id=${encodeURIComponent(threadId)}&limit=50`,
        {
          headers: { Authorization: `Bearer ${apiKey}` },
        },
      );
    ...
    if (path === "/api/webhook/telegram" && req.method === "POST") {
      try {
        ...
        const fromName =
          ...
          " " +
          ((msg.from && msg.from.last_name) || "");
        const name =
          fromName.trim() || (msg.chat && msg.chat.title) || "Telegram User";
      ...
      try {
        ...
        for (const entry of body.entry || []) {
          for (const event of entry.messaging || []) {
            ...
            const senderId =
              event.sender && event.sender.id ? event.sender.id : "unknown";
            ...
            await supabase(env, "/rest/v1/messages", {
              ...
              body: JSON.stringify({
                id:
                  "msg_" + ((event.message && event.message.mid) || Date.now()),
                user_id: systemUserId,
                conversation_id: convId,
                platform: "messenger",
                sender_type: "contact",
          ...
          for (const change of entry.changes || []) {
            ...
            if (
              val.sender &&
              val.recipient &&
              val.sender.id === val.recipient.id
            )
              continue;
            const senderId =
              val.sender && val.sender.id ? val.sender.id : "unknown";
            const text = (val.message && val.message.text) || "[media]";
            const ts = new Date(
              (val.timestamp || 0) * 1000 || Date.now(),
            ).toISOString();
    ...
    if (path === "/api/imap/store" && req.method === "POST") {
      try {
        ...
        if (!provider || !acctEmail || !app_password)
          return json(
            { ok: false, error: "provider, email, app_password required" },
            400,
          );
    ...
    if (path === "/api/sentinel/capture" && req.method === "POST") {
      try {
        ...
        const item = {
          id:
            "sen_" + Date.now() + "_" + Math.random().toString(36).slice(2, 7),
          url: body.url || "",
        ...
        const existing = JSON.parse(
          (await env.LIFEOS_KV.get("sentinel_items")) || "[]",
        );
