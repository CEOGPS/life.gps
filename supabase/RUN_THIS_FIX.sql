-- ============================================================================
-- LifeOS1 — Fix Conflicting Contacts Table & Create All Dashboard Tables
-- Run this ENTIRE script in Supabase SQL Editor
-- ============================================================================

-- 1. DROP the old contacts table (migration 0002 uses user_id UUID, we need user_email TEXT)
DROP TABLE IF EXISTS contacts CASCADE;

-- 2. Now create all dashboard module tables with user_email TEXT schema
...
-- CONTACTS TABLE (with user_email TEXT, matching LifeOSDataContext)
...
-- 3. Also create user_data table for usePersistentState (from FIX_PERSISTENT_MEMORY.sql)
...
-- 4. user_settings table (from 0001_create_user_settings.sql)
...
CREATE OR REPLACE FUNCTION user_settings_touch_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW(); RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_user_settings_updated_at ON user_settings;
CREATE TRIGGER trg_user_settings_updated_at BEFORE UPDATE ON user_settings FOR EACH ROW EXECUTE FUNCTION user_settings_touch_updated_at();
...
WHERE table_schema = 'public' 
  AND table_name IN (
    'user_data', 'user_settings', 'tasks', 'notes', 'calendar_events', 
    'leads', 'budget_bills', 'notifications', 'activity_events',
    'quick_links', 'contacts'
  )
ORDER BY table_name;