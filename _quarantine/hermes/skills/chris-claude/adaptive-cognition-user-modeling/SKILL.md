---
name: adaptive-cognition-user-modeling
description: Use when modeling Chris preferences, communication style, and recurring corrections; adaptive learning loops and persona updates for LifeOS/Hermes.
version: 1.0.0
...
metadata:
  hermes:
    ...
    related_skills: []
    source: C:/dev/.claude/skills/claude-build-adaptive-cognition-and-user-modeling-engine
---
...
## Chris Green — Known Preferences (Embedded)

**Response Style:** Max 10 lines unless showing code or technical detail. User explicitly corrected verbose output: "Reply in 10 lines or less, unless we are dealing with code."

**Communication:** Blunt, iterative, cheap/local models, practical. Values brevity, direct feedback, and clear push-back on weak ideas. "Think Deep and Respond Concise": avoid info-avalanches, one step at a time.

**Workflow:** Prefers precise, iterative fixes. LifeOS dashboard mockup = placement/alignment only, not pixel clone. Colors: black bg, glass, crimson/gray/white/red; red module hover + dot grid.

**Hermes routing:** Prefer free/cheap (OpenRouter :free, local Ollama) over burning Grok credits on infra/debug. Frustrated when local fails or agents retry slow tool loops.

**Dashboard rebuilds:** Preserve full module inventory + wiring; non-uniform spans OK, not a minimal subset. When asked for the list, deliver full inventory in-chat immediately—not only paths or "continue Part N".

**Fix priority:** Global CSS (index.css, PanelShell) before big feature wiring: full-width panels, glass/crimson like dashboard—no 1280px centering on every route.
